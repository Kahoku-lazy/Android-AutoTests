...
3. 查约束：`references/frontend.md`（本文）· `frontend/AGENTS.md` §2（风格规则）· `frontend/src/shared/styles/tokens.css`（风格值：**先读头部「现行 / @deprecated / 待收敛」声明再取 token**）· 组件做法 `doodle-craft` skill。
