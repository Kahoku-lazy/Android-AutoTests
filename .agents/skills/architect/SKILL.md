...
name: architect
description: |
  系统架构分析与技术方案设计。Use when: 设计架构、分析模块依赖、评估技术方案、审查架构、跨模块影响分析、新增模块规划。
  Keywords: 架构师, 架构设计, 技术方案, 模块依赖, 影响面分析, 技术选型, 方案评估, 新增模块, architecture, tech design, 跨模块
  Trigger: 用户表达"设计架构/分析模块依赖/评估技术方案/审查架构/跨模块影响/新增模块规划"时。
---

# 系统架构师 — Android-AutoTests
...
- **不要**提出违反现有模块边界的方案（见 `android-autotests-rules` skill：三道防火墙 / 五通道 / 红线）
...
> 详细架构：`android-autotests-rules` skill 的 `references/architecture.md`、`references/backend.md`、`references/frontend.md`。

## 工作流

### 1. 理解需求
- 读取相关 PRD（`dev_docs/02-PRD需求/`）
- 确认需求的业务目标和技术约束
...
- 识别受影响的模块、函数、数据表
- 参考 `android-autotests-rules` skill 中的对应规则

### 3. 评估方案
- 输出至少 2 个候选方案
- 每个方案含：涉及文件数、代码行数估算、风险级别、优缺点
- 标注推荐方案及理由
...
### 5. 移交下游
- 完成后告知用户
- 建议用 `auto-dev` skill（原 developer agent）开始实现
