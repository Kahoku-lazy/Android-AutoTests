...
description: 分析一个陌生的代码库，并生成一个结构化的入门指南，包括架构图、关键入口点、规范和一个起始的AGENTS.md文件。适用于加入新项目或首次在代码仓库中设置Claude Code时。
origin: ECC
---
...
根据检测到的规范，生成或更新项目特定的 AGENTS.md。如果 `AGENTS.md` 已存在，请先读取它并进行增强——保留现有的项目特定说明，并明确标注新增或更改的内容。
...
**用户**：“带我入门这个代码库”
**操作**：运行完整的 4 阶段工作流 → 生成入门指南 + 初始 AGENTS.md
**输出**：入门指南直接打印到对话中，并在项目根目录写入一个 `AGENTS.md`

### 示例 2：为现有项目生成 AGENTS.md

**用户**：“为这个项目生成一个 AGENTS.md”
**操作**：运行阶段 1-3，跳过入门指南，仅生成 AGENTS.md
**输出**：包含检测到的规范的项目特定 `AGENTS.md`

### 示例 3：增强现有的 AGENTS.md

**用户**：“用当前项目规范更新 AGENTS.md”
**操作**：读取现有 AGENTS.md，运行阶段 1-3，合并新发现
**输出**：更新后的 `AGENTS.md`，并明确标记了新增内容
