...
本 skill 的脚本 **不使用平台代码**：设备发现走 `adb`，连接与取数走 `uiautomator2` 自身 API，解析与候选生成走 `scripts/vendor/`（平台算法层的一次性内联副本）。因此：

- 在未初始化平台（无 Django 设置、无依赖注入）的机器上同样可运行；
- skill 不会触碰 `engines/` 的裸句柄，也不受平台分层边界约束（它是工具，不是平台代码）；
- 代价是解析与候选生成各有一份内联副本：平台改了这两处算法，skill **不会自动跟随**。vendor 文件头记了来源路径与来源文件摘要（sha256 前 12 位），需要同步时按来源文件覆盖并更新摘要。
