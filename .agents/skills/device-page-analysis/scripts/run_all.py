"""三步一条命令：采集 → 分层配图 → 生成 HTML 报告（不依赖平台代码）。"""
...
def main() -> None:
    ...
    if args.page_dir:
        ...
        workdir = Path(args.workdir) if args.workdir else capture_page.default_workdir()
        captured = capture_page.capture(args.serial, workdir, args.label)
