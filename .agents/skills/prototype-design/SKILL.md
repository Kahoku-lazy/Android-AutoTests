...
1. **定位 PRD 文档**：
   - 如果用户指定了 PRD 路径，直接读取
   - 如果未指定，先向用户询问 PRD 文档路径，不要自行假设目录
   - 提取产品定位、目标用户、功能需求、交互设计章节

2. **定位低保真 wireframe**：
   - 使用用户给定的 wireframe 目录（与 PRD 处于同一工程根下）
...
4. **文件输出位置**：用户给定工程根下的 `hifi-wireframes/` 目录
...
3. **写入本地文件**：
   - 将 `gen_html` 返回的 HTML 写入 `hifi-wireframes/` 目录
...
**默认：写入本地文件**
- 将高保真原型写入工程根下的 `hifi-wireframes/` 目录
