...
2. **测试范围收敛：禁止主动跑全量回归**。默认只跑与改动直接相关的最小测试集（改到哪个文件就只跑对应用例 / 覆盖该模块的用例）。
   - **禁止**自行发起全量回归：前端全量单元测试（`frontend` 的 `npm test` / 无参 `vitest run`）、仓库门禁 `python run.py check`、`pytest tests/` 等全量套件，都不允许主动跑。
   - 确实需要全量回归时（例如改动跨模块、要关单验收、用户明确要看全绿），**必须先向用户提出**：说清为什么必须全量、预计耗时，得到同意后才执行；用户未同意就不跑。
   - 同一份全量结果不重复跑第二次。


## 代码规范

规范只写**可机械验证**的要求：凡是工具能判定的，一律以工具配置为准；写不进工具、也无法查收的规则不进本文件。

### 1. 格式类要求以工具配置为准

格式、引号、缩进、换行、import 顺序等要求，一律以工具配置为唯一真相源，本文件不重复描述。入口：
