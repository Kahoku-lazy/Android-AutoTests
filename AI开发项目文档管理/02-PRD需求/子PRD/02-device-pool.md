...
> 版本：v2.1 · 状态：⏳ v2 计划（F-07 待开发） · 日期：2026-07-09
...
### 3.7 F-07：局域网（网络）连接设备 · 🚧 待开发

> 对应用户故事 **US-DP-09**。本节将其细化为可开发规格。
...
| 网络断连 | 目标 IP 不可达 / 设备未开启无线调试 | 弹窗保持打开，Toast 显示后端 error | `adb connect` 失败，返回 400 + 中文 error |
...
| Phase 4+ | v2.1 | 局域网（网络）手动连接设备（IP+端口 → adb connect → WIFI 注册） | F-07 🚧 待开发 |
