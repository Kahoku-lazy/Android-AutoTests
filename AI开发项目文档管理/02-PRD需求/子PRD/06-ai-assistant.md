...
> 关联全局：`../全局PRD.md` · 关联架构：`01-技术架构/项目技术架构/当前实现架构方案.md`
...
4. **平台工具集成**：14 个自定义 Tool 封装全部 5 个业务模块 API，智能体可获得完整的设备管理、元素搜索、用例创建、测试执行、报告生成能力
...
| 工具名称 | 对应模块 | 只读 | 描述 |
|---------|---------|:--:|------|
| `get_test_points` | element-locator | ✅ | 获取标记为测试点的元素 |
| `search_elements` | element-locator | ✅ | 按文本/class/resource_id/页面搜索元素 |
...
| `save_report` | report-generator | ❌ | 保存报告记录 |
| `list_reports` | report-generator | ✅ | 列出报告 |
| `search_knowledge_base` | ChromaDB | ✅ | 知识库向量检索（32 篇文档） |
