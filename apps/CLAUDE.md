...
3. 查：`.claude/rules/backend.md`（架构/中间件/端口/公开路径）、`api-conventions.md`（防火墙/信封/新 App 清单）、`python-code.md`（写法/行数上限/职责分层决策树）、`database.md`（表前缀/写路径）、`security.md`（Key 加密）；契约字段对照 → `dev_docs/03-设计与架构/工具-VUE_API_CONTRACT.md`；边界扫描 → `python tools/gen_arch_stats.py --check-boundaries`；**本 App 专属约束 → `apps/{app}/CLAUDE.md`**。
...
> 命名/行数上限/类型注解/迁移纪律等代码编写规范统一收录于 **`.claude/rules/python-code.md`**（架构与配置 → `backend.md`，表与写库 → `database.md`，安全 → `security.md`）；行为决策（改前四步 → 本文件 §0；职责分层决策树 → `python-code.md` §6）。本文件只保留分层边界与契约约束。
...
**App 边界索引（App 专属边界/契约/协议/关单项的唯一落点 → 各 `apps/{app}/CLAUDE.md`）**：

| App | 落点 | 一句话提示 |
|------|------|-----------|
| accounts | `apps/accounts/CLAUDE.md` | 全站鉴权入口，公开路径与中间件一致 |
| dashboard | `apps/dashboard/CLAUDE.md` | 全平台唯一只读聚合区 |
| device_pool | `apps/device_pool/CLAUDE.md` | 设备生命周期 + 状态机，30s 心跳 |
| device_inspector | `apps/device_inspector/CLAUDE.md` | 快照抓取回看（REST），无 WS |
| element_locator | `apps/element_locator/CLAUDE.md` | 三域资产 CRUD |
| case_manager | `apps/case_manager/CLAUDE.md` | 用例定义编排 + 编辑锁 WS |
| test_runner | `apps/test_runner/CLAUDE.md` | 执行状态机 + 进度 WS（10 事件） |
| report_generator | `apps/report_generator/CLAUDE.md` | 报告只读 + FileResponse 下载 |
| workflow | `apps/workflow/CLAUDE.md` | 编排文档 CRUD（legacy 平铺信封） |
| ai_assistant | `apps/ai_assistant/CLAUDE.md` | 对话 + 唯一 SSE + Tool 网关 |
| evaluator | `apps/evaluator/CLAUDE.md` | 评估题库/运行/框架适配（前端 ai-assistant 寄宿） |
...
- **信封特例（legacy 平铺，已登记 ARCH-06/07/09 与前端 `CLAUDE.md` §1.3，禁止新增，未收敛前禁止改造成信封式）**：
...
**WS**：Consumer 必须在 `gateway/routing.py` 注册；事件 `type` 与前端一致；写库仍走 api。仅 2 生产点（§1.2 通道收敛）；事件表见 `apps/test_runner/CLAUDE.md`（10 种）与 `apps/case_manager/CLAUDE.md`（`case_updated`）。

**AI**：Tool 只调各模块 `api.py`；SSE 事件变更同步前端 `CLAUDE.md` §3；依赖 Redis（见 `backend.md`）。
...
[ ] 信封与 snake_case；写失败有 message/日志（App 特例见其 CLAUDE.md）
...
App 级附加项（delta）→ 各 `apps/{app}/CLAUDE.md` 关单段。详细门禁 → skill `django-backend-check`。完整自测命令 → `apps/自测与检测指令.md`。
