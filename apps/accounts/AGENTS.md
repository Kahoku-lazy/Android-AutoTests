...
- 公开路径列表以 `gateway/middleware.py` 的 `_is_public()`（含 `PUBLIC_PATHS` / `RETIRED_AUTH_PATHS`）与 JWTAuthenticationMiddleware 为唯一真相源，**两者必须一致**——改漏导致全站 401 或误放行。
...
无 WS / SSE。JWT 共享 `SECRET_KEY`（Django 与 AgentScope 同进程共享，见 `config/env.py` 与 `config/settings.py`）。

## 关单附加项（全局清单的 delta）

```
[ ] 无 token / 坏 token / 正常 token 三条路径验证过（pytest -m api）
[ ] 公开路径列表未改动；若改动，`gateway/middleware.py` 与 `apps/AGENTS.md` §2 同改
