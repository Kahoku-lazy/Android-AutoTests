...
def _build_model(provider: str, model_name: str, api_key: str, base_url: str):
...
def _build_toolkit(agent_model, user_id: str) -> Toolkit:
    ...
    # ── Platform business tools (14 tools from tool_registry) ──
...
def _resolve_skill_paths(agent_model) -> list[str]:
    ...
    if agent_skill_dir.exists():
        paths.append(str(agent_skill_dir))
    return paths
...
def _build_context_config(agent_model) -> ContextConfig:
    ...
    if agent_model.compression_enabled:
        cfg.trigger_ratio = (agent_model.compression_threshold or 10000) / 100000.0
        cfg.reserve_ratio = 0.3
    return cfg
