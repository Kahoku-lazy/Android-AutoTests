...
from .api_api import save_api_definition, batch_save_api_definitions
...
def api_testing_definitions_handler(request):
    """GET/POST /api/cases/api-testing/definitions."""
    if request.method == "GET":
        directory_id = request.GET.get("directory_id")
        qs = ApiTestCase.objects.order_by("category", "title")
        ...
        defs = [_serialize_api(row) for row in qs]
        ...
        existing = (ApiTestCase.objects.filter(directory=directory, title=title)
                    .exclude(id=case_id).first())
        if existing:
            dir_name = directory.name if directory else "根级（未分类）"
            return JsonResponse({
                "ok": False,
                "error": f"目录「{dir_name}」下已存在同名 API 用例「{title}」（ID: {existing.id}）",
            }, status=409)

        client_updated_at = data.get("updated_at")
        if case_id and client_updated_at:
            current = ApiTestCase.objects.filter(id=case_id).only("id", "updated_at").first()
            if current and current.updated_at:
                ...
                if client_ts and client_ts != db_ts:
                    return JsonResponse({
                        "ok": False,
                        "error": f"API 用例「{title or case_id}」已被他人修改，请刷新后重试",
                    }, status=409)

        ApiTestCase.objects.update_or_create(id=case_id, defaults=defaults)
        updated = ApiTestCase.objects.filter(id=case_id).only(
            "id", "updated_at", "title"
        ).first()
...
def api_testing_definition_detail(request, case_id):
    """GET/DELETE /api/cases/api-testing/definitions/{case_id}."""
    if request.method == "GET":
        try:
            row = ApiTestCase.objects.get(id=case_id)
        ...
        return JsonResponse({"ok": True, "definition": _serialize_api(row)})

    elif request.method == "DELETE":
        ApiTestCase.objects.filter(id=case_id).delete()
...
def api_testing_definitions_batch(request):
    ...
    result = batch_save_api_definitions(normalized, overwrite=overwrite)
