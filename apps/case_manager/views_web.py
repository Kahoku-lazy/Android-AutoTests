...
from .api_web import save_web_definition, batch_save_web_definitions
...
def _serialize_web(row):
    d = {
        ...
        "locked": row.locked, "visibility": row.visibility,
        ...
        "design_method": row.design_method, "metrics": row.metrics,
    }
    return d
...
def web_definitions_handler(request):
    if request.method == "GET":
        ...
        qs = WebTestCase.objects.order_by("category", "title")
        ...
        if directory_id and directory_id.isdigit():
            ...
            sub_ids = CaseDirectory.objects.filter(parent_id=int(directory_id)).values_list("id", flat=True)
            dir_ids.extend(sub_ids)
            qs = qs.filter(directory_id__in=dir_ids)
        defs = [_serialize_web(row) for row in qs]
        ...
        current_user = _resolve_username(getattr(request, 'user_id', None))
        is_new_case = not WebTestCase.objects.filter(id=case_id).exists()

        defaults = {
            "title": data.get("title", ""), "category": data.get("category", ""),
            "description": data.get("description", ""),
            "enabled": bool(data.get("enabled", True)),
            "directory": directory, "priority": data.get("priority", "P1"),
            "url": data.get("url", ""), "precondition": data.get("precondition", ""),
            "steps": data.get("steps", ""), "expected_result": data.get("expected_result", ""),
            "custom_columns": data.get("custom_columns", []),
            "rows": data.get("rows", []),
            "design_method": data.get("design_method", ""), "metrics": data.get("metrics", ""),
        ...
        existing = (WebTestCase.objects.filter(directory=directory, title=title)
                    .exclude(id=case_id).first())
        if existing:
            dir_name = directory.name if directory else "根级（未分类）"
            return JsonResponse({"ok": False, "error": f"目录「{dir_name}」下已存在同名 Web 用例「{title}」（ID: {existing.id}）"}, status=409)

        client_updated_at = data.get("updated_at")
        if case_id and client_updated_at:
            current = WebTestCase.objects.filter(id=case_id).only("id", "updated_at").first()
            if current and current.updated_at:
                ...
                if client_ts and client_ts != db_ts:
                    return JsonResponse({"ok": False, "error": f"Web 用例「{title or case_id}」已被他人修改，请刷新后重试"}, status=409)

        WebTestCase.objects.update_or_create(id=case_id, defaults=defaults)
        updated = WebTestCase.objects.filter(id=case_id).only("id", "updated_at", "title").first()
...
def web_definition_detail(request, case_id):
    if request.method == "GET":
        try:
            row = WebTestCase.objects.get(id=case_id)
        ...
        if row.visibility == "restricted":
            ...
            if row.created_by != current_user and current_user not in permitted:
                return JsonResponse({"ok": False, "error": "not found"}, status=404)
        return JsonResponse({"ok": True, "definition": _serialize_web(row)})
    elif request.method == "DELETE":
        WebTestCase.objects.filter(id=case_id).delete()
...
def web_definitions_batch(request):
    ...
    for c in cases:
        normalized.append({
            ...
            "description": c.get("description", ""), "enabled": c.get("enabled", False),
            "directory_id": directory_id, "priority": c.get("priority", "P1"),
            "url": c.get("url", ""), "precondition": c.get("precondition", ""),
            "steps": c.get("steps", ""), "expected_result": c.get("expected_result", ""),
            "custom_columns": c.get("custom_columns", []),
            "design_method": c.get("design_method", ""), "metrics": c.get("metrics", ""),
        })
    result = batch_save_web_definitions(normalized, overwrite=overwrite)
