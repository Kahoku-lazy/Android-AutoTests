...
def get_descendant_ids(page_id: int) -> set[int]:
    """包含自身在内的所有后代节点 ID。"""
    result = {page_id}
...
def subtree_max_depth(page_id: int) -> int:
...
def validate_move(page_id: int, new_parent_id: int | None) -> None:
    try:
    ...
    if new_parent_id:
        if new_parent_id in get_descendant_ids(page_id):
    ...
    if page.is_folder:
        ...
        deepest = new_depth + subtree_max_depth(page_id) - 1
        if deepest > MAX_PAGE_TREE_DEPTH:
            raise ValueError(f"目录最多嵌套 {MAX_PAGE_TREE_DEPTH} 层")


def move_page(page_id: int, new_parent_id: int | None) -> Page:
    validate_move(page_id, new_parent_id)
    Page.objects.filter(id=page_id).update(parent_id=new_parent_id)
    return Page.objects.get(pk=page_id)


def filter_top_level_page_ids(page_ids: list[int]) -> list[int]:
...
def batch_move_pages(page_ids: list[int], new_parent_id: int | None) -> dict:
    ...
    moved = 0
    errors: list[dict] = []
    for pid in filter_top_level_page_ids(page_ids):
        try:
            move_page(pid, new_parent_id)
            moved += 1
