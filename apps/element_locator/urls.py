"""element-locator URL routing —— DRF router 是 projects / directories / flows 的**唯一实现**。
...
Web / Web 分组 / API 分组 / API 接口 / Web 跳转流五组路由随元素定位的 Web、API
两域整体下线而移除（变更 remove-element-locator-web-api）。
...
from .views_drf import PageFlowViewSet
from .views_page_element_batch import batch_add_elements, batch_delete_elements
...
urlpatterns = [
    ...
    path("batch-move/", batch_move_items, name="el_batch_move"),
    path("batch-delete/", batch_delete_items, name="el_batch_delete"),
    ...
    path("items/batch-delete/", batch_delete_elements, name="el_items_batch_delete"),
