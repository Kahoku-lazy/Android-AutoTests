...
from apps.test_runner.api import resolve_creator
...
def _resolve_run_row(r, task_map):
    ...
    return {
        ...
        "creator": resolve_creator(tc.creator) if tc else None,
...
def _filter_run_queryset(request):
    ...
    if task_name:
        task_ids = TaskCard.objects.filter(name__icontains=task_name).values_list(
            "task_id", flat=True
        )
        qs = qs.filter(client_task_id__in=task_ids)
    if creator:
        task_ids = TaskCard.objects.filter(creator__icontains=creator).values_list(
            "task_id", flat=True
        )
        qs = qs.filter(client_task_id__in=task_ids)
    ...
    return JsonResponse(
        {
            ...
            "bug_summary": {
                k: v
                for k, v in _build_bug_summary(_collect_case_groups(request, "fail")[0]).items()
                if k != "cases"
            },
            "trend": trend,
            "runs": runs,
...
def run_report(request, run_id):
    ...
    recent_qs = TestRunRecord.objects.annotate(
        total=Count("results"),
        passed=Count("results", filter=PASS_Q),
    ).order_by("-id")[:10]
    ...
    # ── Step-level execution details (screenshots + per-step results) ──
    all_step_details = []
    for tr in results_qs:
        for d in tr.step_details or []:
            d.setdefault("caseId", tr.case_id)
            case_title = title_map.get(str(tr.case_id), tr.case_id or "unknown")
            d.setdefault("caseTitle", case_title)
            d.setdefault("_date", str(tr.created_at)[:19] if tr.created_at else "")
        all_step_details.extend(tr.step_details or [])
    ...
    return JsonResponse(
        {
            ...
            "run": {
                ...
                "task_creator": resolve_creator(task_card.creator) if task_card else None,
