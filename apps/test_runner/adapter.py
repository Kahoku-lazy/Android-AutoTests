...
class DeviceAdapter:
    ...
    def drag(self, xpath: str, direction: str, distance: int = 300):
        """Get element center via u2 XPath, then perform Airtest swipe drag."""
        try:
    ...
    def kill_and_start(self, sleep_after_start: float = 3):
        ...
        self.kill_app(pkg)
        self.sleep(2)
        self.log(f"Starting app: {pkg}")
        self.ad.start_app(pkg)
    ...
    def wait_for_either(self, xpath_a: str, xpath_b: str,
        ...
        """
        self.log(f'Polling two elements (timeout {timeout}s)...')
        deadline = time.time() + timeout
        while time.time() < deadline:
            if self.stopped():
                return "stopped"
    ...
    def wait_for_either_indexed(self, xpath_a: str, idx_a: int,
                                 ...
                                 timeout: float = 30) -> str:
        self.log(f'Polling two indexed elements (timeout {timeout}s)...')
        deadline = time.time() + timeout
        while time.time() < deadline:
            if self.stopped():
                return "stopped"
