...
class DeviceAdapter:
    ...
    def __init__(
        ...
        """
        Args:
            device_conn:   DeviceConnection with .u2 (uiautomator2)
        ...
        """
        self.engine = device_conn.engine  # UiEngine 协议
    ...
    def run_watchers(self):
        ...
        for w in self._watchers:
            ...
            if not xpath:
                continue
            try:
                if self.engine.exists(xpath):
                    action = w.get("action", "click")
                    if action == "click":
                        self.engine.click_xpath(xpath)
    ...
    def exists(self, xpath: str, timeout: float = 0) -> bool:
        ...
        try:
            if timeout <= 0:
                return self.engine.exists(xpath)
            else:
    ...
    def long_click(self, xpath: str, duration: float = 0.8):
        try:
            self.engine.long_click_xpath(xpath, duration=duration)
        except Exception as e:
    ...
    def swipe(self, direction: str, distance: int = 500):
        try:
            w, h = self.engine.get_resolution()
            cx, cy = w // 2, h // 2
            dirs = {
    ...
    def click_indexed(self, xpath: str, index: int) -> bool:
        return self.engine.click_xpath(xpath, index=index)
    ...
    def wait_for_toast(self, expected_text: str, timeout: float = 15) -> bool:
        ...
        while time.time() < deadline:
            ...
            # as a device crash).
            try:
                if self.engine.exists(f'//*[@text="{expected_text}"]'):
                    return True
            except Exception:
                logger.debug("Toast xpath probe failed for %r, falling back", expected_text[:60])
            try:
                msg = self.engine.get_toast_message()
    ...
    def get_text(self, xpath: str) -> str:
        return self.engine.get_text(xpath)
    ...
    def wait_appear_then_disappear(self, xpath: str, timeout: float = 30) -> bool:
        ...
        while time.time() < deadline:
            if self.stopped():
                return False
            if not self.engine.exists(xpath):
