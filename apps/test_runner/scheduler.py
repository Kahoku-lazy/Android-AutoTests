...
class BaseScheduler:
    ...
    @staticmethod
    def _bridge_log(run_id: str, msg: str, loop):
        """Bridge sync adapter log → async WS callback (mirrors _bridge_ws_log)."""
    ...
    def _mark_device_busy(self, spec: TaskSpec):
        """子类覆写：标记设备 BUSY（仅 DeviceScheduler）"""

    def _mark_device_idle(self, spec: TaskSpec):
    ...
    async def submit(self, spec: TaskSpec):
        ...
        async with self._lock:
            if len(self._running) < self._max_concurrent:
                ...
                _log.info("[%s] queued: %s (position %d)", self._task_type,
                          spec.run_id, len(self._queue))
    ...
    def status(self) -> SchedulerStatus:
        return SchedulerStatus(
            ...
            queue_info=[{"client_task_id": s.client_task_id, "position": i + 1}
        ...
        try:
            ...
            async with self._semaphore:
                ...
                await _execute_tests(
                    spec.run_id, runner, spec.test_cases,
                    spec.loop_count, spec.interval_seconds, serial,
                )
    ...
    def _try_dequeue(self):
        ...
        if self._queue and len(self._running) < self._max_concurrent:
            ...
            _log.info("[%s] dequeue: %s (remaining queue: %d)",
                      self._task_type, spec.run_id, len(self._queue))
            task = asyncio.create_task(self._run(spec))
            self._running[spec.run_id] = task
            task.add_done_callback(lambda t: self._running.pop(spec.run_id, None))
...
class DeviceScheduler(BaseScheduler):
    ...
    async def _make_runner(self, spec: TaskSpec):
        ...
        from .callbacks import test_callbacks
        from .runner import TestRunner, _device_executor as _u2_executor
        from .executors.ui.connect import check_and_connect_async, DeviceCheckError
    ...
    def _mark_device_busy(self, spec: TaskSpec):
        ...
        try:
            dp_acquire_device(self._serial, user_id=f"runner-{self._serial}", timeout=3600)
        except ValueError:
            _log.warning("[DeviceScheduler] dp_acquire_device failed for %s (already locked?)",
                         self._serial)
    ...
    def _mark_device_idle(self, spec: TaskSpec):
        ...
        try:
            ...
            sync_to_async(dp_release_device)(self._serial, reason="manual")
        except Exception:
...
class ApiScheduler(BaseScheduler):
    """API 调度器：全局 1 个实例，max_concurrent=10。无设备状态管理。"""

    def __init__(self):
        super().__init__(max_concurrent=10, task_type="api_testing")
    ...
    async def _make_runner(self, spec: TaskSpec):
        ...
        adapter = ApiAdapter(
            base_url=base_url,
            logger=lambda msg, l=loop: self._bridge_log(spec.run_id, msg, l),
            should_stop=lambda: False,
        )
        executor = ApiExecutor(adapter)
        return RemoteTestRunner(adapter, executor, device_label="api",
                                callback=test_callbacks)


class WebScheduler(BaseScheduler):
    """Web 调度器：全局 1 个实例，max_concurrent=3。管理 Playwright 浏览器实例。"""

    def __init__(self):
        super().__init__(max_concurrent=3, task_type="web_automation")
    ...
    async def _make_runner(self, spec: TaskSpec):
        ...
        return RemoteTestRunner(adapter, executor, device_label="web",
                                callback=test_callbacks,
...
class SchedulerRegistry:
    ...
    def global_status(self) -> dict:
        return {
            ...
            "devices": {s: sch.status.to_dict()
                        for s, sch in self._device_schedulers.items()},
        }
