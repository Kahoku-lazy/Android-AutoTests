...
async def start_test_run(request):
    ...
    for serial in serials:
        ...
        if is_device_busy(serial):
            ...
            if client_task_id:
                await sync_to_async(
                    TaskCard.objects.filter(task_id=client_task_id).update
        ...
        # Claim device immediately to close the race before runner.run() marks busy
        ...
        try:
            ...
            await test_callbacks.on_log(
                f"queue_{serial}", f"❌ 设备 {serial} 连接失败: {e}")
            continue
        ...
        asyncio.create_task(delayed_execute(run_id, runner, test_cases, loop_count, interval_seconds, start_at, end_at))
...
async def _execute_tests(run_id: str, runner, test_cases: list, loop_count: int, interval_seconds: int = 5, serial: str = ""):
    run_record = None
    run_completed = False
    ...
    try:
        ...
        def create_run_record():
            ...
            for tc in test_cases:
                snapshots.append({
                    "case_id": tc.id,
                    "title": tc.title,
                    "steps_data": [s.to_dict() if hasattr(s, 'to_dict') else (s.__dict__ if hasattr(s, '__dict__') else str(s)) for s in (tc.steps_data or [])],
                })
            ...
            return TestRunRecord.objects.create(
                ...
                client_task_id=ctid,
                status='RUNNING',
                device_serial=serial or (runner.device.serial if hasattr(runner.device, 'serial') else ''),
        ...
        if client_tid:
            ...
            def _mark_done():
                ...
                TaskCard.objects.filter(task_id=client_tid).update(
                    status='done', running=False, outcome=final_outcome,
                    case_items=case_items,
                    overall_pass=overall_pass, overall_fail=overall_fail,
                )
            await _mark_done()
        ...
        if client_tid:
            ...
            def _mark_error():
                TaskCard.objects.filter(task_id=client_tid).update(
                    status='done', running=False, outcome='error',
                )
            try:
                await _mark_error()
            except Exception:
...
async def _start_next_queued(serial: str):
    ...
    try:
        ...
        await _execute_tests(
            ...
            next_req.get("interval_seconds", 5), serial)
    except Exception as e:
    ...
    if active_runs_empty:
        ...
        for tc in db_queued:
            ...
            already_enqueued = any(
                item.get("client_task_id") == tc.task_id
                ...
                try:
                    ...
                    if _loop.is_running():
                        _asyncio.run_coroutine_threadsafe(
                            _start_next_queued(serial), _loop
                        )
                except RuntimeError:
                    pass
...
def task_card_save(request):
    ...
    task, created = TaskCard.objects.update_or_create(
