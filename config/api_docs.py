...
ENDPOINTS = [
    ...
    {
        "module": "element-locator", "prefix": "/api/elements", "desc": "元素定位：UI dump、8种XPath策略、截图叠加层、页面跳转",
        "items": [
            {"method":"POST","path":"/api/elements/dump","desc":"Dump 当前 UI 层级并持久化到数据库",
             "body":{"action":"dump"},"body_desc":"空 JSON 对象即可",
             "response":{"ok":"bool","page_id":"int","package":"str","activity":"str","element_count":"int","actionable_count":"int","elements":"list[dict]","actionable":"list[dict]"},
             "errors":[{"code":500,"msg":"设备未连接或 uiautomator2 异常"}],
             "note":"核心操作，每次切换页面后需要调用。自动截图保存到 data/screenshots/，为每个可定位元素生成8种XPath候选"},
            {"method":"POST","path":"/api/elements/action","desc":"执行点击或输入操作","body":{"action":"click|input","x":"int","y":"int","text":"str(输入时)","clear_first":"bool"},
             "response":{"ok":"bool"},"errors":[{"code":500,"msg":"设备未连接或操作失败"}],"note":"click 只需 x,y；input 需要 x,y,text,clear_first"},
            {"method":"GET","path":"/api/elements/pages","desc":"已 dump 的页面列表(含出入度)","body":None,
             "response":{"ok":"bool","pages":"list[{id,label,package,activity,element_count,created_at,flow_out,flow_in}]"},"errors":[],"note":"按创建时间倒序"},
            {"method":"PUT","path":"/api/elements/pages/{page_id}","desc":"更新页面标签","body":{"label":"str"},"response":{"ok":"bool"},"errors":[],"note":"page_id 来自 dump 返回或页面列表"},
            {"method":"DELETE","path":"/api/elements/pages/{page_id}","desc":"删除页面及关联的元素和跳转","body":None,"response":{"ok":"bool"},"errors":[],"note":"级联删除"},
            {"method":"POST","path":"/api/elements/pages/clear","desc":"清空全部页面/元素/跳转数据","body":None,"response":{"ok":"bool"},"errors":[],"note":"不可逆"},
            {"method":"GET","path":"/api/elements/pages/{page_id}/items","desc":"页面元素列表(支持过滤)","body":None,"response":{"ok":"bool","elements":"list[dict]"},
             "errors":[],"note":"Query: ?filter=all(默认)|clickable|text|testpoint"},
            {"method":"PUT","path":"/api/elements/items/{el_id}","desc":"更新元素元数据","body":{"alias":"str","tags":"str","is_test_point":"bool","notes":"str"},
             "response":{"ok":"bool"},"errors":[],"note":"标记测试点后可在 YAML 导出中使用"},
            {"method":"GET","path":"/api/elements/flows","desc":"页面跳转关系列表","body":None,
             "response":{"ok":"bool","flows":"list[{id,from_page_id,to_page_id,from_label,to_label,trigger_text,trigger_rid}]"},"errors":[],"note":"LEFT JOIN 页面和元素表"},
            {"method":"POST","path":"/api/elements/flows","desc":"创建页面跳转记录","body":{"from_page_id":"int","to_page_id":"int","trigger_element_id":"int","trigger_action":"str"},
             "response":{"ok":"bool"},"errors":[],"note":"trigger_action 默认 click"},
            {"method":"DELETE","path":"/api/elements/flows/{flow_id}","desc":"删除跳转记录","body":None,"response":{"ok":"bool"},"errors":[],"note":""},
        ],
    },
    # ── device-pool (10 endpoints) ──
    {
        "module": "device-pool", "prefix": "/api/devices", "desc": "设备管理：ADB扫描、状态监控(ONLINE/BUSY/OFFLINE)、锁定释放、排队、心跳",
        "items": [
            {"method":"GET","path":"/api/devices","desc":"设备列表 + 状态 + 排队数","body":None,
             "response":{"ok":"bool","devices":"list[{serial,model,screen,sdk,status,locked_by,locked_at}]","current":"str","queue_length":"int"},
             "errors":[],"note":"每次调用自动触发心跳检测，同步 adb 真实状态到 DB。BUSY 设备超时自动释放"},
            {"method":"POST","path":"/api/devices/scan","desc":"扫描并连接设备(无线ADB或USB)","body":{"address":"str (IP:port 或 USB串号)"},
             "response":{"ok":"bool","message":"str","serial":"str"},"errors":[{"code":400,"msg":"address 为空"},{"code":500,"msg":"adb 不可用"}],"note":"包含:的地址走 adb connect；不含:的走 adb devices 查找"},
            {"method":"POST","path":"/api/devices/{serial}","desc":"连接设备(自动切换为当前)","body":{"activate":"bool(默认true)"},
             "response":{"ok":"bool","serial":"str","connected":"bool"},"errors":[{"code":400,"msg":"设备不可达"}],"note":"连接成功后自动执行 device_pool.switch_to(serial)"},
            {"method":"GET","path":"/api/devices/current","desc":"当前活动设备信息","body":None,
             "response":{"serial":"str","screen_w":"int","screen_h":"int","package":"str"},"errors":[],"note":"不依赖 adb，从 DevicePool 缓存读取"},
            {"method":"POST","path":"/api/devices/{serial}/disconnect","desc":"断开设备(同时释放锁)","body":None,"response":{"ok":"bool"},"errors":[],"note":"无线设备执行 adb disconnect；清理 u2 连接缓存；若设备被锁则自动释放"},
            {"method":"POST","path":"/api/devices/{serial}/activate","desc":"切换当前活动设备","body":None,"response":{"ok":"bool","current":"str"},"errors":[],"note":"后续所有 dump/action 操作针对此设备"},
            {"method":"POST","path":"/api/devices/{serial}/lock","desc":"锁定设备(独占使用)","body":{"user_id":"str","timeout":"int(秒,默认300)"},
             "response":{"ok":"bool","serial":"str","locked_by":"str","timeout":"int"},"errors":[{"code":409,"msg":"设备已被其他用户锁定"}],"note":"锁定后设备状态变为 BUSY；超时后自动释放。同一设备重复锁定会检查超时"},
            {"method":"POST","path":"/api/devices/{serial}/release","desc":"释放设备锁","body":None,
             "response":{"ok":"bool","serial":"str","status":"ONLINE"},"errors":[],"note":"状态恢复为 ONLINE；清空锁定信息；删除锁记录"},
            {"method":"GET","path":"/api/devices/queue","desc":"排队状态(活跃锁列表)","body":None,
             "response":{"ok":"bool","queue_length":"int","queue":"list[{serial,user_id,locked_at,timeout_seconds,remaining_seconds,position}]"},"errors":[],"note":"FIFO 排序，包含剩余超时时间"},
            {"method":"GET","path":"/api/devices/heartbeat","desc":"手动触发心跳检测","body":None,
             "response":{"ok":"bool","online":"int","busy":"int","offline":"int","total":"int"},"errors":[],"note":"检查所有已注册设备状态，同步 adb 真实连接，清理过期锁。前端每30秒自动调用"},
        ],
    },
    # ── case-manager (8 endpoints) ──
    {
        "module": "case-manager", "prefix": "/api/cases", "desc": "用例工程：用例CRUD、步骤编排(14种)、YAML导出、测试套件",
        "items": [
            {"method":"GET","path":"/api/cases/definitions","desc":"测试用例定义列表","body":None,
             "response":{"ok":"bool","definitions":"list[{id,title,category,description,steps,enabled,steps_data,package_name,created_at,updated_at}]"},"errors":[],"note":"steps_data 是 TestStep 对象数组，按 category+title 排序"},
            {"method":"POST","path":"/api/cases/definitions","desc":"创建或更新用例定义","body":{"id":"str(必填)","title":"str","category":"str","description":"str","steps_data":"list","enabled":"bool","package_name":"str"},
             "response":{"ok":"bool","id":"str"},"errors":[{"code":400,"msg":"id 为空"}],"note":"id 已存在则更新，不存在则插入。steps_data 为 TestStep JSON 数组"},
            {"method":"GET","path":"/api/cases/definitions/{case_id}","desc":"单个用例详情","body":None,"response":{"ok":"bool","definition":"dict"},"errors":[{"code":404,"msg":"case_id 不存在"}],"note":""},
            {"method":"DELETE","path":"/api/cases/definitions/{case_id}","desc":"删除用例定义","body":None,"response":{"ok":"bool"},"errors":[],"note":""},
            {"method":"POST","path":"/api/cases/export/yaml","desc":"导出测试点为 YAML 文件","body":{"test_case_name":"str","page_ids":"list[int]"},
             "response":{"ok":"bool","filename":"str","yaml":"str"},"errors":[],"note":"收集标记为 is_test_point 的元素，生成 YAML 格式测试用例。同时缓存在 cm_test_cases 表"},
            {"method":"GET","path":"/api/cases/exports","desc":"已导出的 YAML 文件列表","body":None,"response":{"ok":"bool","files":"list[{name,size,time}]"},"errors":[],"note":"按时间倒序"},
            {"method":"GET","path":"/api/cases/exports/{filename}","desc":"下载 YAML 导出文件","body":None,"response":"FileResponse (application/x-yaml)","errors":[{"code":404,"msg":"文件不存在"}],"note":"不是 JSON 响应，前端需用 fetch().then(r=>r.text())"},
            {"method":"POST","path":"/api/cases/suites","desc":"创建测试套件 (v2)","body":"待定义","response":"待定义","errors":[],"note":"⏳ v2 计划"},
        ],
    },
    # ── test-runner (4 endpoints) ──
    {
        "module": "test-runner", "prefix": "/api/runner", "desc": "执行引擎：异步测试执行、14种步骤类型、WebSocket 实时进度、CSV/MD报告",
        "items": [
            {"method":"POST","path":"/api/runner/run","desc":"启动测试执行(异步)","body":{"case_ids":"list[str]","loop_count":"int(默认10)","package_name":"str"},
             "response":{"ok":"bool","run_id":"str","case_count":"int","loop_count":"int","ws_url":"str"},
             "errors":[{"code":400,"msg":"case_ids 为空"},{"code":404,"msg":"无启用的用例"}],
             "note":"核心端点。后台 asyncio.create_task() 异步执行，立即返回 run_id。通过 ws_url 建立 WebSocket 接收实时进度"},
            {"method":"POST","path":"/api/runner/run/{run_id}/stop","desc":"停止运行中的测试","body":None,
             "response":{"ok":"bool","message":"str"},"errors":[{"code":404,"msg":"run_id 不存在或已结束"}],"note":"优雅停止：当前步骤完成后终止"},
            {"method":"GET","path":"/api/runner/run/{run_id}/status","desc":"查询测试执行进度","body":None,
             "response":{"ok":"bool","status":"str(pending|running|stopped|completed)","is_running":"bool","selected_cases":"list","loop_count":"int","started_at":"str"},
             "errors":[{"code":404,"msg":"运行记录未找到"}],"note":"优先查活跃运行(内存)，其次查历史 DB"},
            {"method":"GET","path":"/api/runner/runs","desc":"执行历史列表(最近50条)","body":None,
             "response":{"ok":"bool","runs":"list[{run_id,total,passed,failed,last_time}]"},"errors":[],"note":"从 tr_test_results 表聚合统计"},
        ],
    },
    # ── report-generator (2 endpoints) ──
    {
        "module": "report-generator", "prefix": "/api/reports", "desc": "报告分析：CSV/MD/JSON 报告浏览与下载",
        "items": [
            {"method":"GET","path":"/api/reports","desc":"可下载的报告文件列表","body":None,
             "response":{"ok":"bool","files":"list[{name,size,time,type}]"},"errors":[],"note":"type: csv|md|log。从 logs/ 目录扫描 result_* 前缀文件"},
            {"method":"GET","path":"/api/reports/{filename}","desc":"下载报告文件","body":None,
    ...
    # ── WebSocket (2 endpoints) ──
    {
        ...
        "items": [
            {"method":"WS","path":"/ws/screenshot","desc":"实时截图流 2fps","body":None,
             "response":"S→C: {\"type\":\"screenshot\",\"image\":\"<base64 PNG>\"}","errors":[],"note":"连接后自动开始推送。无客户端时停止截图。Vite proxy 已配置"},
            {"method":"WS","path":"/ws/test-run/{run_id}","desc":"测试执行实时进度","body":None,
             "response":"6 种消息: log|case_started|iteration_result|case_finished|run_finished|device_error","errors":[],"note":"run_id 来自 POST /api/runner/run 返回值。断开连接后回调自动清理"},
        ],
    },
]
