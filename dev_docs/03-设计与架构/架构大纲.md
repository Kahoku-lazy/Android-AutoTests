...
> 本文档是平台的**架构总纲**，定义三层分离设计意图、前后端与 AI 交互边界、八模块架构全景。
...
flowchart TB
    ...
    subgraph L3["💾 数据与设备层"]
        direction LR
        DB[("SQLite / MySQL<br/>22 张业务表")]
        Redis[("Redis :6379<br/>消息总线")]
        Android["📱 Android 设备<br/>uiautomator2 + ADB"]
    end
    ...
    Django_L2 --> DB
    Django_L2 -->|"uiautomator2"| Android
    AS_L2 --> Redis
    Django_L2 --> Redis
...
│  │  · 业务逻辑 (views.py)      │  │  · 24 个 Tool 调用编排       ││
│  │  · ORM 数据持久化           │  │  · ChromaDB 知识库检索       ││
│  │  · uiautomator2 设备控制    │  │  · Agent Team 并行协作       ││
...
flowchart LR
    ...
    Django -->|"⑤ ADB<br/>uiautomator2 设备控制"| Dev
    API --> DB
    Tool --> DB
...
|:--:|------|------|------|------|------|------|
| ① | 前端 → Django | 双向 | HTTP REST | JWT Bearer Header | 业务 CRUD（106 REST 端点） | 低带宽，请求-响应 |
...
flowchart TB
    ...
    subgraph Backend["⚙️ 后端 Django :8765"]
        direction TB
        Views["views.py × 8<br/>106 REST 端点"]
...
| **API** | 13 REST |
| **核心组件** | `DevicePool` 单例 (pool.py) — 线程安全的 u2 连接管理 |
...
| 维度 | 说明 |
|------|------|
| **模块职责** | 二级目录树 → 用例 CRUD → 17 种步骤编排 → YAML 导入导出 |
...
| **下游消费** | 无（AI 是顶层调用者） |
| **AI Tool** | 全部 24 个 Tool（AI 模块自身也是 Tool 的宿主） |
...
│
├── 项目架构.md                           ← 原始架构文档（保留参考）
├── 模块-AI助手-技术架构与功能设计.md      ← AI 助手历史设计文档
├── 模块-元素定位-元素管理页UI改动总结.md   ← 元素定位 UI 改动记录
│
├── 技术栈-README.md                      ← 技术选型与素材约定
├── 技术栈-命名统一标准.md                ← 命名规范
├── 技术栈-animal-island-ui设计素材.md     ← 动森 design token
├── 技术栈-PROMPT-animal-island-ui一键提示词.md
│
├── 工具-VUE_API_CONTRACT.md              ← 前后端接口契约
├── Harness_Loop_Engineering_方案.md      ← Harness Loop 工程方案
│
└── README.md                             ← 目录索引
```
...
| 用例管理 | [`ARCH-03-用例管理.md`](ARCH-03-用例管理.md) | 17 种步骤编排 + 二级目录树 + YAML 导入导出 |
...
| `el_` | `element_locator` | `el_pages` · `el_elements` · `el_page_flows` |
...
**17 种步骤类型** — `StepType` 枚举是唯一真相源，executor dispatch、前端 step-utils、adapter 均基于这些值：
