...
| 业务组件 | `DeviceCard`（拍立得设备卡）、`DeviceStatusCell`（状态列 el-tag + Badge）、`DeviceActionsCell`（操作列按钮组）、`QueuePanel`（排队 Popover）、`DisconnectDialog`（强制断开确认）、`NetworkConnectDialog`（局域网连接表单） |
...
| 架构约束 | **device-pool 是管理模块（有写操作）**，区别于 dashboard 的只读聚合：scan/connect/lock/release/disconnect/queue 均走 `api.ts` → djangoClient，组件不直连 HTTP |
...
| 12px | `var(--app-size-xs)` ✅ | doc-tag、章节 label、筛选标签、视图切换、工具栏、表头、Badge、操作按钮、图钉区序列号、排队 badge |
...
| 更浅分割线（表格 td、离线照片区底） | `var(--app-border-lighter)` | #f0ede8 |
| 微妙底色（表头底、卡片组计数底、禁用态） | `var(--app-bg-subtle)` | #f8f6f2 |
| 离线灰（OFFLINE 状态） | `var(--app-offline)` | #d4d8dc |
...
| 排队 / 断开语义色 | `var(--app-queue-text)` / `var(--app-disconnect-text)` | #b08800 / #c53030 |
...
| `OFFLINE` | info | 离线 · 不可用 | 灰：`--app-border-lighter` 底 + `--app-ink-muted` 字 + `--app-offline` 边 |
| `DISCONNECTED` | danger | 已断开 · 不可用 | 粉：`--app-status-danger-bg` 底 + `--app-status-danger-text` 字 |

> ⚠️ 注意 `warning` type 被覆盖为粉（BUSY 与 DISCONNECTED 同粉），`info` 被覆盖为灰。改状态色时需同步 DeviceStatusCell.vue 的 `:deep(.el-tag--*)` 覆盖块。

状态 Badge（执行中/占用中/已绑定）映射：执行中=粉（danger）、占用中=黄（`--app-status-warning-bg` + `--c-dashboard` 边）、已绑定=紫（`--app-status-purple-bg` + `--app-status-purple-border`）。
...
| 离线 | `#d4d8dc`（硬编码，应 `var(--app-offline)`） | square | OFFLINE+DISCONNECTED |
| 总计 | `var(--ink)` ✅ | circle | 全部 |

> ⚠️ 前三张卡硬编码 hex，与「总计」的 token 写法不一致，见 §8 已知偏差 #1。
...
| 风格元素 | 规格 | 应用处 |
|---------|------|--------|
| 拍立得设备卡 | 3px 状态色边（绿/粉/灰）+ 圆角 6px 10px + `::before` 图钉（9×9 径向渐变圆，居中置顶）+ 扁平投影 | DeviceCard |
...
| 排队 badge | 粉底 `--app-status-danger` + 2px ink 边 + 扁平投影 | QueuePanel 触发钮 |
...
| KPI 网格 | `repeat(4, 1fr)` gap 18px；**≤900px 断点** → 2 列 |
...
| 卡片视图 | `card-group-grid` `repeat(3, 1fr)` gap 14px；**≤900px** → 2 列；按状态分组（🟢在线 / 🔴使用中 / ⚫离线） |
...
| 端点（11） | `GET /devices`、`POST /devices/scan`、`POST /devices/{serial}`（connect）、`POST /devices/{serial}/activate`、`POST /devices/{serial}/lock`、`POST /devices/{serial}/release`、`POST /devices/{serial}/disconnect`、`GET /devices/queue`、`POST /devices/{serial}/queue`、`POST /devices/{serial}/queue/leave`、`GET /devices/heartbeat` |
| 字段（snake_case） | `serial` / `model` / `brand` / `screen` / `status` / `connection_type` / `locked_by` / `occupied_by` / `last_seen` / `user_id` / `waited_seconds` |
| 状态枚举 | `ONLINE` / `BUSY` / `OFFLINE` / `DISCONNECTED`（`DEVICE_STATUS_MAP` 是唯一映射源） |
...
- [ ] 写操作（锁定/排队/断开）走 api.ts 通道，组件无 fetch/axios
- [ ] 检查测试未依赖被改样式（`grep -rn "device-card\|device-section\|device-table\|status-cell\|action-bar\|queue-badge" frontend/tests/`）
...
| # | 位置 | 偏差 | 建议 |
|---|------|------|------|
| 1 | `index.vue` KPI 卡 `#6BCB77`/`#FFB5A7`/`#d4d8dc` | 硬编码 hex（「总计」却用 `var(--ink)`，写法不一致） | 换 `var(--app-status-success/-danger)` / `var(--app-offline)` |
...
| 5 | `QueuePanel.vue` `rgba(0,0,0,0.05)` | 排队 badge 阴影硬编码 | 换 `--app-shadow-sm` |
| 6 | `NetworkConnectDialog.vue` `#e8998a` + `rgba(232,153,138,0.14)` | 错误红硬编码（`--ac-red` fallback 带硬编码值） | 收敛为状态 token（如 `--app-error`） |
| 7 | `FilterTabs.vue`（共享组件）`#e8ecf1`/`#FFE066` | 共享组件硬编码（非本模块） | 与仪表盘/侧边栏终审同法收敛，本模块不重复登记 |
