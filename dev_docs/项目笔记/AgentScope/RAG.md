...
<Tip>
  嵌入模型的介绍和配置方式请见[嵌入模型章节](/versions/2.0.8dev/zh/building-blocks/model/embedding)；服务化版本的 RAG（带 HTTP 服务、文件托管、分布式索引）请见 [RAG 服务](/versions/2.0.8dev/zh/deploy/rag)。
</Tip>
...
请见[嵌入模型章节](/versions/2.0.8dev/zh/building-blocks/model/embedding)。
...
<Tip>
  `agentscope[rag]` 只包含文档解析所需的依赖，向量数据库各有独立的可选依赖：`agentscope[vdb-qdrant]`、`agentscope[vdb-milvus]`、`agentscope[vdb-mongodb]`、`agentscope[vdb-elasticsearch]`。
</Tip>
...
<Steps>
  ...
  <Step title="写入知识库">
    ...
    <CodeGroup>
      ...
      # 本地持久化 .db 文件；需要 agentscope[vdb-milvus]
      ...
      # MongoDB Vector Search（Atlas 或自建集群）；需要 agentscope[vdb-mongodb]
      store = MongoDBStore(
      ...
      # 稠密向量索引 + 近似 kNN 检索；需要 agentscope[vdb-elasticsearch]
...
async with store:
    results = await knowledge.search(
        ...
        top_k=3,
        score_threshold=None,  # 命中分数下限，None 表示不过滤
    )
    for r in results:
...
<Note>
  `score` 统一为**越大越相关**：使用距离度量（如 L2）的向量库会在返回前取负，因此这类知识库的分数为负值，`score_threshold` 也要相应地填负数。分数只在同一个知识库内部可比，不同嵌入模型或不同度量的知识库之间没有可比性。
</Note>
...
| `score_threshold`    | `None`      | 命中分数下限，`None` 表示不过滤；分数越大越相关，距离度量下为负值                              |
| `rerank_candidate_k` | `None`      | 交给重排模型判断的候选数，从中保留 `top_k` 条；必须不小于 `top_k`，默认为其两倍，上限 50。未配置重排模型时忽略 |
...
<CodeGroup>
  ...
  static_mw = RAGMiddleware(
      knowledge_bases=[knowledge],       # 一个或多个 KnowledgeBase 句柄
...
向量检索按嵌入相似度排序，语义上更贴近问题的片段未必排在前面。给 `RAGMiddleware` 传入 `rerank_model`，检索会先取回 `rerank_candidate_k` 条候选，再由该模型读完内容后挑出最终的 `top_k` 条：

```python 配置重排模型 theme={null}
from agentscope.middleware import RAGMiddleware
...
rag_mw = RAGMiddleware(
    knowledge_bases=[knowledge],
    rerank_model=DashScopeChatModel(       # 承担重排的对话模型
        credential=DashScopeCredential(api_key="YOUR_API_KEY"),
        model="qwen-max",
    ),
    parameters=RAGMiddleware.Parameters(
        mode="agentic",
        top_k=3,                # 最终保留 3 条
        rerank_candidate_k=10,  # 先取 10 条候选交给重排模型
    ),
)
```
...
| 特性    | 说明                                                                                 |
| ----- | ---------------------------------------------------------------------------------- |
| 候选数量  | `rerank_candidate_k` 必须不小于 `top_k`，默认取 `2 × top_k`，上限 50；候选太少时重排只能在向量检索已经返回的结果里换顺序 |
| 失败处理  | 重排是尽力而为的，模型调用失败时记录一条警告并回退到向量检索的顺序，不会中断本次检索                                         |
| 多模态候选 | 重排模型无法阅读的候选（如它不支持的模态）会被排到最后或直接剔除                                                   |
| 提示词   | `rerank_prompt` 可以覆盖重排指令，必须包含 `{query}` 占位符，可选 `{top_k}`；候选内容由框架以内容块的形式追加在指令之后     |

<Warning>
  重排每次检索都会额外调用一次大模型。候选数越多、内容越长，延迟与成本越高，建议从默认的 `2 × top_k` 开始调整。
</Warning>
