...
import {
  ...
  fetchAgentTools,
  saveMcp as saveMcpApi,
  testMcpConnection,
  uploadSkill as uploadSkillApi,
  toggleToolEnabled,
  deleteToolById,
} from "./api.js";
...
function deselectAllSkills() {
  const cfg = {};
  availableSkills.value.forEach((s) => {
    cfg[s.name] = false;
  });
  form.value.skills_config = cfg;
}
...
function isDocEnabled(docId) {
  ...
  if (sources.includes("__none__")) return false; // explicit none
  if (!sources.length) return true; // empty = all enabled (backward compat)
...
function toggleDoc(docId) {
  ...
  // Remove __none__ sentinel — toggling any doc exits "none" state
  sources = sources.filter((s) => s !== "__none__");

  if (wasNone) {
    // Transitioning from "none": enable just this doc
    sources = [docId];
  ...
  // Normalize: all enabled → [], none enabled → ['__none__']
  const allIds = new Set(knowledgeDocs.value.map((d) => d.id));
  const selectedIds = new Set(sources);
  if (sources.length === 0) {
    form.value.knowledge_sources = ["__none__"];
  } else if (
    allIds.size === selectedIds.size &&
  ...
  if (mode === "edit" && index >= 0) {
    ...
    const cfgStr =
      typeof t.config_json === "string"
        ? t.config_json
        : JSON.stringify(t.config || t.config_json || {}, null, 2);
...
async function saveMcpTool() {
  ...
  if (!raw) {
    mcpJsonError.value = "JSON 配置不能为空";
    return;
  }

  const normalized = normalizeMcpJson(raw);
  if (!normalized) {
    mcpJsonError.value = "JSON 格式无效，请检查语法";
    return;
  }
...
async function removeMcpApi(index) {
  ...
  if (!tool) return;
  try {
    await ElMessageBox.confirm(`确定删除 MCP「${tool.name}」？`, "确认删除", {
      type: "warning",
    });
  } catch {
    return;
...
async function testMcp(index) {
  ...
  if (isNew.value && typeof cfgStr !== "string")
    cfgStr = JSON.stringify(cfgStr);
  let cfgObj;
  try {
...
async function toggleMcp(i) {
  if (isNew.value) {
    form.value.tools[i].enabled = !form.value.tools[i].enabled;
    return;
  }
...
async function handleSkillFolderChange(e) {
  ...
  try {
    ...
    if (data.ok) {
      ...
      ElMessage.success(
        `Skill「${folderName}」上传成功 (${data.data.config?.file_count || 0} 个文件)`,
      );
    } else {
...
async function removeSkill(index) {
  ...
  try {
    await ElMessageBox.confirm(
      `确定删除 Skill「${skill.name}」？相关文件将被清除。`,
      "确认删除",
      { type: "warning" },
    );
  } catch {
    ...
    <div class="doc-body agent-body">
      ...
      <div v-if="!isNew || step === 2" class="doc-section step-panel">
        ...
        <el-form label-width="110px" class="agent-form">
          ...
          <el-form-item label="模型名称">
            <el-select
              v-if="form.model_provider !== 'custom'"
          ...
          <el-form-item label="生成参数">
            <el-input
              v-model="form.generate_kwargs"
              type="textarea"
              :rows="2"
              placeholder='{"parallel_tool_calls":true}'
            />
          </el-form-item>
        </el-form>
      </div>
      ...
      <div v-if="!isNew || step === 4" class="doc-section step-panel">
        ...
        <el-dialog
          ...
          :title="
            mcpDialogMode === 'add' ? '添加 MCP 服务器' : '编辑 MCP 服务器'
          "
          width="560px"
          destroy-on-close
          append-to-body
          ...
          <el-form label-width="80px">
            ...
            <el-form-item label="JSON 配置" required>
              <div style="width: 100%">
                ...
                <div
                  v-if="mcpJsonError"
                  style="color: #e85f5f; font-size: 12px; margin-top: 4px"
                >
                  {{ mcpJsonError }}
                </div>
                <div
                  v-if="mcpForm.config_json.trim()"
                  class="json-preview-live"
                  style="
                    margin-top: 8px;
                    max-height: 200px;
                    overflow: auto;
                ...
                >
                  <pre
                    v-html="highlightJson(mcpForm.config_json)"
                    style="
                      margin: 0;
        ...
        <el-form label-width="120px" class="agent-form">
          ...
          <template v-if="form.compression_enabled">
            ...
            <el-form-item label="保留最近">
              <el-input-number
                v-model="form.compression_keep_recent"
                :min="1"
                :max="50"
              />
            ...
            <el-form-item label="压缩提示词">
              <el-input
                v-model="form.compression_prompt"
                type="textarea"
                :rows="2"
            ...
            <el-form-item label="摘要模板">
              <el-input
                v-model="form.compression_template"
                type="textarea"
                :rows="2"
                placeholder="Previous summary: {summary}"
              />
            </el-form-item>
          </template>
        </el-form>
