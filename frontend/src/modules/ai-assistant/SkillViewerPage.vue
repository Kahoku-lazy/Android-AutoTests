      ...
      <div v-else class="sv-page">
        <aside class="sv-tree panel">
          ...
          <div v-loading="treeLoading" class="sv-tree-body">
            <EmptyState
              v-if="!treeLoading && !tree.length"
              icon="📁"
              text="该 Skill 没有可预览文件"
            />
            <el-tree
              v-else
              :data="tree"
            ...
            >
              <template #default="{ data }">
                <span
                  class="sv-node"
                  :class="{ active: selectedPath === data.path && !data.is_dir }"
                >
        ...
        <section class="sv-content panel">
          ...
          <div v-else v-loading="fileLoading" class="sv-content-body">
            ...
            <EmptyState
              ...
              text="文件过大，无法在页面中预览"
            />
            <article v-else-if="file?.kind === 'markdown'" class="sv-md" v-html="markdownHtml" />
