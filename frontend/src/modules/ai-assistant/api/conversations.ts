...
/** 删除对话 */
export async function deleteConversation(convId: number): Promise<{ status: boolean; data?: object; message?: string }> {
  const { data } = await djangoClient.post<{ status: boolean; data?: object; message?: string }>(`/ai/conversations/${convId}/delete`)
  return data
}
...
/** 发送 HITL 确认结果 */
export async function postConfirmResult(convId: number, result: object): Promise<{ status: boolean; data?: object; message?: string }> {
  const { data } = await djangoClient.post<{ status: boolean; data?: object; message?: string }>(
