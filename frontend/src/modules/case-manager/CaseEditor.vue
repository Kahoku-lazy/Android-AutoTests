...
async function loadDevices() {
  try {
    ...
    if (data.ok) {
      devices.value = (data.devices || []).filter(d => d.status === 'ONLINE' || d.status === 'BUSY')
      if (!debugDevice.value && devices.value.length) debugDevice.value = devices.value[0].serial
    }
  } catch (_) {}
}
...
async function save() {
  ...
  if (!form.value.package_name.trim()) {
    ElMessage.warning('请输入 APP 包名（步骤中的启动/关闭/重启应用需要此信息）')
    return false
  }
  saving.value = true
  try {
    ...
    <div class="doc-body">
      ...
      <section class="doc-section form-section">
        <div class="doc-section__header">
          ...
          <div class="actions">
            <el-button @click="exitPage">
              <Icon name="icon-close" :size="14" style="margin-right:4px" />退出
            </el-button>
            <el-button type="primary" :loading="saving" @click="save">
              <Icon name="icon-check" :size="14" style="margin-right:4px" />保存
            </el-button>
          </div>
        </div>
          ...
          <el-row :gutter="24">
            <el-col :span="14">
              <el-form-item label="用例 ID">
                <el-input v-model="form.id" :disabled="!isNew" placeholder="留空则自动生成 TC-日期-时间-随机码">
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="12">
              <el-form-item label="分类">
                <el-input v-model="form.category" placeholder="如：登录、支付、首页" />
              </el-form-item>
            </el-col>
          </el-row>

          <el-form-item label="包名" required>
            <el-input v-model="form.package_name" placeholder="com.example.app（必填，启动/关闭/重启应用时使用）" />
          </el-form-item>
      ...
      <section class="doc-section steps-section">
        <div class="doc-section__header">
          ...
          <div class="steps-header-actions">
            <el-select v-model="debugDevice" size="small" placeholder="选择调试设备" style="width:260px" @focus="loadDevices">
              <el-option v-for="d in devices" :key="d.serial" :label="`${d.model || d.serial} [${d.serial}]`" :value="d.serial">
                <span>{{ d.model || d.serial }}</span>
                <span style="float:right;color:#9f927d;font-size:12px">{{ d.serial }}</span>
              </el-option>
            </el-select>
            <AnimalButton type="primary" size="small" @click="goToElementLocator">
              <Icon name="icon-search" :size="14" style="margin-right:4px" />去元素定位
            </AnimalButton>
          </div>
        </div>
        <div class="doc-section__label">点击、滑动、等待、校验等操作步骤，选择设备后可单步或批量调试执行</div>
        <StepEditor v-model="form.steps_data" :debug-device="debugDevice" :package-name="form.package_name" />
      </section>
    </div>
  </div>
</template>
...
<style scoped>
.case-editor-page { min-height: 100%; }
