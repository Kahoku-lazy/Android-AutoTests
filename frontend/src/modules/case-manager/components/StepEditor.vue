...
import {
  IconPlus, IconRefresh, IconTrash, IconGripVertical,
...
const STEP_TYPES = [
  { value: 'click',          label: '点击元素',       icon: '👆', desc: '点击屏幕上的一个元素', group: '元素操作' },
  { value: 'long_click',     label: '长按元素',       icon: '🖐', desc: '按住元素不放，持续N秒后松开', group: '元素操作' },
  { value: 'click_indexed',  label: '点击第N个元素',  icon: '👆', desc: '多个相同元素时，点击其中第N个', group: '元素操作' },
  { value: 'swipe',          label: '滑动屏幕',       icon: '👈', desc: '在屏幕上向某个方向滑动一段距离', group: '滑动操作' },
  { value: 'drag',           label: '拖动元素',       icon: '✋', desc: '按住一个元素，向某个方向拖动', group: '滑动操作' },
  { value: 'wait',           label: '等待元素出现',   icon: '⏳', desc: '等待某个元素出现在屏幕上，超时则失败', group: '等待操作' },
  { value: 'wait_disappear', label: '等待元素消失',   icon: '⏳', desc: '等待某个元素从屏幕上消失，超时则失败', group: '等待操作' },
  { value: 'wait_any',       label: '等待任意一个出现', icon: '⏳', desc: '等待多个元素中任意一个出现即可', group: '等待操作' },
  { value: 'wait_toast',     label: '等待Toast消息',  icon: '💬', desc: '等待系统弹出指定的Toast提示', group: '等待操作' },
  { value: 'sleep',          label: '暂停几秒',       icon: '😴', desc: '什么都不做，暂停等待一段时间', group: '等待操作' },
  { value: 'verify_text',    label: '校验文字',       icon: '✅', desc: '检查元素的文字内容是否等于预期值', group: '检查操作' },
  { value: 'poll_text',      label: '等待文字出现',   icon: '🔄', desc: '等待元素出现并且文字等于预期值', group: '检查操作' },
  { value: 'start_app',      label: '打开应用',       icon: '🚀', desc: '启动手机上的某个应用', group: '应用操作' },
  { value: 'kill_app',       label: '关闭应用',       icon: '💀', desc: '强制退出手机上的某个应用', group: '应用操作' },
  { value: 'restart_app',    label: '重启应用',       icon: '🔄', desc: '先关闭应用，再重新打开', group: '应用操作' },
  { value: 'retry_click',    label: '点击后等待结果', icon: '🔁', desc: '点击一个元素，然后等待另一个元素出现确认生效', group: '应用操作' },
  { value: 'log',            label: '记录信息',       icon: '📝', desc: '在测试日志中记录一条文字信息', group: '应用操作' },
]
...
const FIELD_LABELS = {
  xpath: '目标元素', xpath2: '确认元素',
  timeout: '超时(秒)', expected_text: '预期文本',
  index: '索引', description: '描述',
  direction: '方向', distance: '距离(像素)',
}
...
function defaultStep(type = 'click') {
  const xpath = APP_LIFECYCLE_TYPES.includes(type) ? (props.packageName || '') : ''
  return { type, xpath, xpath2: '', timeout: 10, expected_text: '', index: 0, description: '', direction: 'up', distance: 500 }
}
...
function onDragLeave() { dropTargetIdx.value = null }

function onDragEnd() { dragIndex.value = null; dropTargetIdx.value = null }

function onDrop(e, idx) {
  e.preventDefault(); e.stopPropagation()
  if (dragIndex.value === null || dragIndex.value === idx) {
    dragIndex.value = null; dropTargetIdx.value = null; return
  }
  const arr = [...steps.value]
  const [moved] = arr.splice(dragIndex.value, 1)
  arr.splice(idx, 0, moved)
  steps.value = arr; expanded.value = {}
  dragIndex.value = null; dropTargetIdx.value = null
}
...
function fieldLabel(step, field) {
  if (field === 'xpath' && ['start_app','kill_app','restart_app'].includes(step.type)) return '包名'
...
function fieldHint(step, field) {
  if (field === 'xpath' && step.type === 'wait_any') return '多个XPath用 | 分隔，任意一个出现即通过'
  if (field === 'xpath' && step.type === 'long_click') return '选择要长按的元素'
  if (field === 'timeout' && step.type === 'long_click') return '按住不放的秒数'
...
function useElementPicker(step) {
  return !['start_app','kill_app','restart_app','log','swipe'].includes(step.type)
}
...
async function runStep(idx, step) {
  if (!props.debugDevice) { ElMessage.warning('请先在顶部选择调试设备'); return }
  runningStep.value = idx
  stepResults.value[idx] = null
  try {
    const { data } = await client.post('/runner/run-step', { device_serial: props.debugDevice, type: step.type, xpath: step.xpath || '', xpath2: step.xpath2 || '', timeout: step.timeout || 10, expected_text: step.expected_text || '', index: step.index ?? 0, direction: step.direction || '', distance: step.distance || 500, description: step.description || '' })
    stepResults.value[idx] = data.ok ? { ok: true, message: data.message || '执行成功' } : { ok: false, message: data.error || '执行失败' }
  } catch (e) { stepResults.value[idx] = { ok: false, message: e.response?.data?.error || e.message || '请求失败' } }
  runningStep.value = null
}

async function runStepsRange(fromIdx) {
  runningBatch.value = true; runningFromIdx.value = fromIdx
  ...
  for (let i = 0; i < stepsToRun.length; i++) {
    ...
    try {
      ...
      const { data } = await client.post('/runner/run-step', { device_serial: props.debugDevice, type: s.type, xpath: s.xpath || '', xpath2: s.xpath2 || '', timeout: s.timeout || 10, expected_text: s.expected_text || '', index: s.index ?? 0, direction: s.direction || '', distance: s.distance || 500, description: s.description || '' })
      stepResults.value[realIdx] = data.ok ? { ok: true, message: data.message || '执行成功' } : { ok: false, message: data.error || '执行失败' }
      if (!data.ok) {
        try { await ElMessageBox.confirm('上一步执行失败，是否继续？', '步骤失败', { confirmButtonText: '继续', cancelButtonText: '停止', type: 'warning' }) }
        catch (_) { break }
      }
    } catch (e) { stepResults.value[realIdx] = { ok: false, message: e.message || '请求失败' }; break }
  }
  runningStep.value = null; runningBatch.value = false; runningFromIdx.value = null
}
  ...
  <div class="step-editor">
    <div class="step-header">
      ...
      <div class="step-header__actions">
        <template v-if="steps.length">
          <el-button size="small" type="success" plain :loading="runningBatch && runningFromIdx === 0" @click="runAllSteps" :disabled="runningBatch">▶▶ 从头执行</el-button>
    ...
    >
      ...
      <div class="step-bar" @click="expanded[idx] = !expanded[idx]">
        ...
        <span class="step-summary">
          {{ stepSummary(step) }}
          <span v-if="stepResults[idx]" :class="['step-result-msg', stepResults[idx].ok ? 'ok' : 'fail']">{{ stepResults[idx].message }}</span>
        </span>
        <span class="step-actions" @click.stop>
          <el-button size="small" type="success" plain :loading="runningStep === idx" @click="runStep(idx, step)" title="单步执行">▶</el-button>
          <el-button size="small" type="warning" plain :loading="runningFromIdx === idx" @click="runFromCurrent(idx)" title="从这步开始执行到结束" :disabled="runningBatch && runningFromIdx !== idx">▶▶ ▸</el-button>
          <el-button size="small" plain @click="duplicateStep(idx)" title="复制步骤">📋</el-button>
          <el-button size="small" type="danger" plain @click="requestRemoveStep(idx)">
      ...
      <div v-show="expanded[idx]" class="step-form">
        <div class="step-desc">{{ STEP_TYPES.find(t=>t.value===step.type)?.desc }}</div>
        <el-form label-width="80px" size="small">
          <el-form-item label="类型">
            <el-select v-model="step.type" @change="(t) => onTypeChange(idx, t)" style="width: 240px">
              <el-option-group
                v-for="grp in [...new Set(STEP_TYPES.map(s=>s.group))]" :key="grp"
                :label="grp"
              >
                <el-option v-for="st in STEP_TYPES.filter(s=>s.group===grp)"
                  :key="st.value" :label="st.icon + ' ' + st.label" :value="st.value" />
              </el-option-group>
            </el-select>
          </el-form-item>
          ...
          <template v-for="field in visibleFields(step)" :key="field">
            <!-- Direction → select -->
            <el-form-item v-if="field === 'direction'"
              :label="FIELD_LABELS.direction" :required="isFieldRequired(step, field)">
            ...
            <!-- Distance → number -->
            <el-form-item v-else-if="field === 'distance'"
              :label="FIELD_LABELS.distance" :required="isFieldRequired(step, field)">
            ...
            <el-form-item
              v-else-if="(field === 'xpath' || field === 'xpath2') && useElementPicker(step)"
            ...
            >
              <el-select
                ...
                filterable :filter-method="(q) => elementSearchQuery = q"
                clearable style="width:100%"
                @change="(val) => onElementPicked(step, field, val)"
              >
                <el-option-group
                  v-for="pg in filteredPages" :key="pg.id"
                  :label="(pg.label || 'Page#'+pg.id) + ' (' + (filteredElements.filter(e=>e._pageId===pg.id).length) + ')'"
                >
                  <el-option v-for="el in filteredElements.filter(e => e._pageId === pg.id)" :key="el.id"
                    :label="elementOptionLabel(el)" :value="el.id">
                    <div class="el-opt">
                      <span class="el-opt-name">{{ el.alias || el.text_val || el.resource_id || '未命名' }}</span>
                      <span class="el-opt-page">{{ pg.label || 'Page#'+pg.id }}</span>
                    </div>
                  </el-option>
                </el-option-group>
              </el-select>
            ...
            <el-form-item
              v-else-if="(field === 'xpath' || field === 'xpath2') && !useElementPicker(step)"
              :label="fieldLabel(step, field)" :required="isFieldRequired(step, field)">
            ...
            <el-form-item
              ...
              :label="FIELD_LABELS[field]" :required="isFieldRequired(step, field)">
          ...
          <el-form-item label="描述" v-if="!visibleFields(step).includes('description')">
    ...
    >
      ...
      <template #footer>
        ...
        <AnimalButton type="danger" @click="confirmRemove">确定删除</AnimalButton>
      </template>
    </Modal>
  </div>
</template>
...
.step-header__title { font-size: 14px; font-weight: 600; color: #725d42; }
.step-header__actions { display: flex; gap: 8px; }
...
.step-item:hover { border-color: #d0c8b8; box-shadow: 0 2px 8px rgba(61, 52, 40, 0.08); }
.step-item.expanded { border-color: var(--accent-blue); box-shadow: 0 2px 10px rgba(64, 158, 255, 0.1); }
.step-item.dragging { opacity: 0.4; transform: scale(0.97); box-shadow: 0 0 0 2px #19c8b9; }
.step-item.drop-target { border-color: #19c8b9 !important; box-shadow: 0 0 0 2px #19c8b9, 0 4px 16px rgba(25,200,185,0.25) !important; }
.step-item.drop-target .step-bar { background: #e6f9f6; }
...
.drag-handle:hover { color: #19c8b9; background: rgba(25,200,185,0.08); }
.drag-handle:active { cursor: grabbing; }
.step-item.dragging .drag-handle { color: #19c8b9; }
.step-idx {
  width: 24px;
  height: 24px;
  border-radius: 50%;
...
.step-summary { flex: 1; font-size: 14px; color: #5c4b38; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.step-actions { display: flex; gap: 6px; flex-shrink: 0; }
.step-actions :deep(.el-button) { padding: 5px 8px; }
.step-result-msg { font-size: 11px; margin-left: 8px; font-weight: 600; }
.step-result-msg.ok { color: #6fba2c; }
.step-result-msg.fail { color: #e85f5f; }
...
.add-bottom { margin-top: 14px; text-align: center; }
.field-hint { font-size: 12px; color: #a0936e; margin-left: 10px; }
.current-xpath { margin-top: 6px; font-size: 12px; }
.current-xpath code {
  background: #f5f3ed;
  padding: 3px 8px;
  border-radius: 4px;
