...
const DEFAULT_COLS = [
  ...
  { key: "headers", label: "请求头", width: 250, editable: true },
  { key: "body", label: "请求体", width: 250, editable: true },
  { key: "expected_response", label: "预期响应文本", width: 250, editable: true },
];
...
async function loadDirectories() {
  try {
    const { data } = await fetchDirectories("api_testing");
    ...
    if (data.ok) {
      ...
      if (!rows.value.length && d.title) {
        rows.value.push({
          ...
          headers: d.headers, body: d.body, expected_response: d.expected_response,
        });
        rowCounter = 2;
      } else {
...
async function acquireLock() {
  ...
  try {
    ...
    if (data.ok) { editLockHeld.value = true; lockHeartbeat = setInterval(async () => { try { await acquireEditLock(caseId.value); } catch (_) {} }, 600000); }
  } catch (e) { if (e.response?.status === 423) lockOwner.value = e.response.data.editing_by || "其他用户"; }
}
...
onBeforeRouteLeave((_to, _from, next) => {
  if (JSON.stringify({ rows: rows.value, columns: columns.value }) !== originalJson.value) {
...
async function doSave() {
  ...
  try {
    ...
    const payload = {
      title: firstRow.title || `API用例-${new Date().toISOString().slice(0,10)}`,
      priority: firstRow.priority || "P1",
      precondition: firstRow.precondition || "",
      headers: firstRow.headers || "",
      body: firstRow.body || "",
      expected_response: firstRow.expected_response || "",
    ...
    if (data.ok) {
      ...
      if (isNew.value) router.replace(`/cases/api/${data.id}/edit`);
...
<template>
  <div class="doc-page wb-shell">
    <PageHeader :title="isNew ? '新建 API 接口用例' : '编辑 API 接口用例'" icon="🌐">
      <template #actions>
            ...
            <thead>
              <tr>
                ...
                <th v-for="(col, ci) in columns" :key="col.key" :style="{ minWidth: col.width + 'px' }">
                  {{ col.label }}
                  <button v-if="!DEFAULT_COLS.find(dc => dc.key === col.key)" class="btn-col-remove" @click="removeColumn(ci)" title="删除此列">✕</button>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(row, ri) in rows" :key="ri" :class="{ 'row-selected': selectedRow === ri }" @click="selectedRow = ri">
                <td class="col-num">{{ ri + 1 }}</td>
                <td v-for="col in columns" :key="col.key">
                  <input v-if="col.editable" v-model="row[col.key]" class="cell-input" :placeholder="col.label" />
