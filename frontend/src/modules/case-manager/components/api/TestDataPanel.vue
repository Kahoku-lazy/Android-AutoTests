<template>
  <div class="test-data-panel">
    <div class="panel-header">
      ...
      <el-button size="small" type="primary" text @click="addRow">+ 添加行</el-button>
    </div>
            ...
            <tbody>
              <tr v-for="(row, ri) in modelValue" :key="ri">
                ...
                <td class="data-td data-td--output">
                  <button class="output-btn" @click="openOutputEditor(ri)">
                ...
                <td class="data-td data-td--act">
                  <button class="row-del-btn" @click="removeRow(ri)" title="删除行">×</button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <el-dialog v-model="outputDialogVisible" title="行输出断言配置" width="500px" :close-on-click-modal="false">
      <div class="dialog-row">
        ...
        <el-select v-model="editingOutput.step_index" size="small" class="form-group__select--w120">
      ...
      <div class="dialog-row">
        ...
        <div class="mode-switch">
          <el-radio-group v-model="outputSchemaMode" size="small">
        ...
        <div v-if="outputSchemaMode === 'visual'" class="visual-schema">
          <div v-for="(f, fi) in outputFields" :key="fi" class="schema-field">
            <el-input v-model="f.key" size="small" placeholder="字段名" class="form-group__input--w110" @change="syncOutputSchema" />
            <el-select v-model="f.type" size="small" class="form-group__select--w85" @change="syncOutputSchema">
            ...
            <el-checkbox v-model="f.required" size="small" @change="syncOutputSchema">必填</el-checkbox>
            <el-button size="small" text type="danger" @click="removeOutputField(fi)">×</el-button>
          </div>
          <el-button size="small" text @click="addOutputField">+ 字段</el-button>
        </div>
        <el-input
          v-else
