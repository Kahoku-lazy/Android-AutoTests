...
async function loadDefs() {
  loading.value = true;
  try {
    const { data } = await listWebDefinitions(props.activeDirectoryId);
...
const breadcrumbPath = computed(() => {
  ...
  const parts = [];
  function find(node, id) { if (node.id === id) return [node]; if (node.children) for (const c of node.children) { const f = find(c, id); if (f) return [node, ...f]; } return null; }
...
function createCase() {
  router.push({ path: "/cases/web/new", query: props.activeDirectoryId ? { directory_id: props.activeDirectoryId } : {} });
}
function editCase(row) { router.push(`/cases/web/${row.id}/edit`); }
async function doRemove(row) {
  try { await deleteWebDefinition(row.id); ElMessage.success("已删除"); loadDefs(); emit("refresh-tree"); }
  catch (e) { ElMessage.error("删除失败: " + (e?.response?.data?.error || e?.message || "网络错误")); }
}
    ...
    <div class="case-toolbar__left">
      <nav class="case-breadcrumb">
        <button class="crumb crumb--root" :class="{ 'crumb--active': activeDirectoryId === null }" @click="goToAll">
          <span class="crumb__icon">🌍</span><span class="crumb__label">全部 Web 用例</span>
        </button>
  ...
  <div v-else-if="viewMode === 'card'" class="card-grid">
    <AppCard v-for="item in definitions" :key="item.id" class="web-card">
      <div class="web-card__header">
        ...
        <span class="web-card__priority">{{ item.priority }}</span>
      </div>
      <h3 class="web-card__title">{{ item.title }}</h3>
      <p v-if="item.url" class="web-card__url">🌍 {{ item.url }}</p>
      <p v-if="item.description" class="web-card__desc">{{ item.description }}</p>
      <div class="web-card__meta">
...
.case-link { cursor: pointer; color: #6fba2c; }
</style>
