...
async function onDelete(snap) {
  try {
    await ElMessageBox.confirm("删除后该快照与其截图 / 缩略图文件不可恢复", "删除快照", {
      type: "warning",
...
<template>
  <el-drawer v-model="store.drawerVisible" title="检查器快照" size="380px">
    <div class="snap-list">
      ...
      <EmptyState v-if="!store.snapshots.length" text="暂无快照" hint="获取成功后自动保存到这里" />
      <div
      ...
      >
        <div class="snap-main">
          <div class="snap-title">{{ METHOD_LABELS[s.method] || s.method }} · {{ s.serial }}</div>
          <div class="snap-sub">{{ s.package || "—" }} · 元素 {{ s.element_count }}</div>
          <div class="snap-time">
