...
<template>
  <div class="status-cell">
    <el-tag :type="statusTag(device.status).type" size="small" effect="dark" round>{{
      statusTag(device.status).text
    }}</el-tag>
    <el-tooltip v-if="isExecutionOccupied(device)" :content="device.occupied_by" placement="top">
      <span class="badge badge-exec">执行中</span>
    </el-tooltip>
    <el-tooltip v-else-if="device.occupied_by" :content="device.occupied_by" placement="top">
...
.status-cell :deep(.el-tag--success) {
  background: var(--app-status-success-bg) !important;
  color: var(--app-status-success-text) !important;
  border-color: var(--app-status-success) !important;
}
.status-cell :deep(.el-tag--warning) {
...
.badge-process {
  ...
  border: 1px solid var(--c-dashboard);
}
.badge-locked {
  background: var(--app-status-purple-bg);
  color: var(--app-status-purple-text);
  border: 1px solid var(--app-status-purple-border);
}
</style>
