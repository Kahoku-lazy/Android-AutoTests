...
import {
  apiListWebGroups, apiCreateWebGroup, apiUpdateWebGroup, apiDeleteWebGroup,
  apiListWebElements, apiBatchMoveWebGroups,
} from '../api'

/**
 * WebGroup tree state and operations — mirrors useElementTree for el_web_groups.
 */
export function useWebGroupTree() {
  const groups = ref([])
  ...
  async function loadGroups() {
    ...
    try {
      ...
      if (data.status) groups.value = data.groups || []
  ...
  async function selectGroup(groupOrNull) {
    ...
    try {
      ...
      if (data.status) elements.value = data.elements || []
    } catch (e) { console.error(e); }
  }
  ...
  async function doCreateGroup() {
    ...
    try {
      const { data } = await apiCreateWebGroup({
  ...
  async function doRename() {
    ...
    try {
      const { data } = await apiUpdateWebGroup(g.id, { name })
      if (data.status) {
  ...
  async function deleteGroup(g) {
    try {
      await ElMessageBox.confirm(
        `删除「${g.name}」？${g.is_folder ? '子分组也将被删除，元素变为未分类。' : '其中的元素将变为未分类。'}`,
        '确认删除', { confirmButtonText: '确认删除', cancelButtonText: '取消', type: 'warning' },
      )
    } catch { return }
    try {
      const { data } = await apiDeleteWebGroup(g.id)
      if (data.status) {
  ...
  async function handleNodeDrop(draggingNode, dropNode, dropType) {
    ...
    try {
      const { data } = await apiBatchMoveWebGroups([src.id], targetParentId)
      if (data.status) { src.parent_id = targetParentId; loadGroups() }
  ...
  async function confirmBatchMove() {
    ...
    try {
      const { data } = await apiBatchMoveWebGroups([...selectedGroupIds], pid)
      if (data.status) { selectedGroupIds.clear(); selectMode.value = false; loadGroups() }
    } catch (e) { ElMessage.error('移动失败') }
  }
