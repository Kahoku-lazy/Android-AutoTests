          ...
          <div v-if="caseItems.length" class="case-list">
            <div v-for="ci in caseItems" :key="ci.id" class="case-card" :class="{ expanded: expandedCases.has(ci.id) }">
              <div class="case-header" @click="toggleCase(ci.id)">
                ...
                <span class="case-status-text" :style="{ color: ci.fail > 0 ? '#e85f5f' : '#6fba2c', background: ci.fail > 0 ? 'rgba(232,95,95,0.12)' : 'rgba(111,186,44,0.1)' }">
                  {{ ci.fail > 0 ? '🔴 有失败' : '✅ 全部通过' }}
                </span>
...
.step-done .step-strip { background: #6fba2c; }
