...
async function loadReport() {
  ...
  loading.value = false
  await nextTick()
  animate(".task-report-table tbody tr", {
    opacity: [0, 1],
    translateY: [12, 0],
    delay: stagger(30),
    duration: 350,
  ...
  <div v-if="error" class="doc-page wb-shell task-report-page">
    ...
    <div class="doc-body">
      <WorkbenchCrumbs
        back-to="/reports"
        back-label="报告列表"
        :items="[{ label: '测试报告', to: '/reports' }, { label: '任务报告' }]"
      />
    ...
    <div class="doc-body">
      ...
      <div class="kpi-row">
        <KpiCard :value="kpiTotal" label="总迭代次数" color="var(--c-workflow)" shape="diamond">
          <div class="kpi-sub">
            {{ taskMeta.case_ids?.length || 0 }} 用例 × {{ taskMeta.loop_count || 0 }} 轮
          </div>
        </KpiCard>
      ...
      <div v-if="perfStats" class="perf-stats-section">
        ...
        <div class="perf-stats-grid">
          <KpiCard
            :value="perfStats.count"
            label="🔢 测量次数"
            color="var(--c-workflow)"
            shape="square"
          />
          <KpiCard
            :value="`${perfStats.max}s`"
            label="⬆ 最大耗时"
            color="var(--c-runner)"
            shape="triangle"
          />
          <KpiCard
            :value="`${perfStats.min}s`"
            label="⬇ 最小耗时"
            color="var(--c-device)"
            shape="triangle"
          />
          <KpiCard
            :value="`${perfStats.avg}s`"
            label="📊 平均耗时"
            color="var(--c-element)"
            shape="diamond"
          />
          <KpiCard
            :value="`${perfStats.median}s`"
            label="🎯 中位数"
      ...
      <div class="task-meta-card">
        <div class="meta-item">
          ...
          ><span class="meta-value">{{ taskMeta.name }}</span>
        </div>
        <div class="meta-item">
          <span class="meta-label">👤 创建人</span
          ><span class="meta-value">{{ taskMeta.creator }}</span>
        </div>
        <div class="meta-item">
          <span class="meta-label">📱 设备</span
          ><span class="meta-value">{{ taskMeta.device_serial }}</span>
        </div>
        <div class="meta-item">
          <span class="meta-label">⚙ 模式</span
          ><span class="meta-value">{{ taskMeta.mode === "scheduled" ? "定时" : "即时" }}</span>
        </div>
        <div class="meta-item">
          <span class="meta-label">🔁 轮次</span
          ><span class="meta-value"
            >{{ taskMeta.loop_count }} 轮 · 间隔 {{ taskMeta.interval_seconds || 5 }}s</span
          >
        </div>
        <div v-if="taskMeta.round" class="meta-item">
          <span class="meta-label">🔄 重试</span
          ><span class="meta-value">第 {{ taskMeta.round }} 轮</span>
        </div>
      ...
      <AppTabs class="detail-tabs" :items="tabs" v-model="activeTab">
        ...
        <template #cases>
          <div v-if="caseItems.length" class="case-list">
            <div
              v-for="ci in caseItems"
              :key="ci.id"
              class="case-card"
              :class="{ expanded: expandedCases.has(ci.id) }"
            >
              <div
                class="case-header"
                role="button"
                tabindex="0"
                @click="toggleCase(ci.id)"
                @keydown.enter.prevent="toggleCase(ci.id)"
                @keydown.space.prevent="toggleCase(ci.id)"
              >
                ...
                <span
                  class="case-status-text"
                  :style="{
                    color: ci.fail > 0 ? 'var(--app-error)' : 'var(--c-workflow)',
                    background:
                      ci.fail > 0 ? 'var(--rg-status-fail-bg)' : 'var(--rg-status-pass-bg)',
                  }"
                >
                  {{ ci.fail > 0 ? "🔴 有失败" : "✅ 全部通过" }}
                </span>
        ...
        <template v-if="bugEntries.length > 0" #failures>
          <div
            v-for="entry in bugEntries"
            :key="entry.key"
            class="case-card bug-card"
            :class="{ expanded: expandedBugs.has(entry.key) }"
          >
            <div
              class="case-header"
              role="button"
              tabindex="0"
              @click="toggleBug(entry.key)"
              @keydown.enter.prevent="toggleBug(entry.key)"
              @keydown.space.prevent="toggleBug(entry.key)"
            >
              <span class="case-expand-icon">▶</span>
              <span class="case-id-badge" style="background: var(--app-status-danger-text)"
                >BUG-{{ String(entry.bugNum).padStart(3, "0") }}</span
              >
              <div class="case-title-area">
                <span class="case-title-text">{{ entry.caseTitle }}</span>
                <div class="bug-header-sub">
                  第 {{ entry.iteration }} 轮 · 步骤 {{ entry.stepIndex + 1 }} 失败
                </div>
              </div>
              <span
                class="case-status-text"
                style="color: var(--app-status-danger-text); background: var(--rg-status-fail-bg)"
                >🔴 执行失败</span
              >
            </div>
            <div v-if="expandedBugs.has(entry.key)" class="case-body">
              <div class="bug-meta">
                <span
                  ><span class="bug-meta-label">🐛 BUG 编号</span>
                  <span class="bug-meta-value"
                    >BUG-{{ String(entry.bugNum).padStart(3, "0") }}</span
                  ></span
                >
                <span
                  ><span class="bug-meta-label">📋 关联用例</span>
                  <span style="font-weight: 700">{{ entry.caseTitle }}</span></span
                >
                <span
                  ><span class="bug-meta-label">🔁 失败轮次</span>
                  <span class="bug-meta-value">第 {{ entry.iteration }} 轮</span></span
                >
                <span
                  ><span class="bug-meta-label">❌ 失败步骤</span>
                  <span class="bug-meta-value"
                    >步骤 {{ entry.stepIndex + 1 }} · {{ entry.stepType }}</span
                  ></span
                >
              </div>
          ...
          <AppCard color="" class="table-card">
            ...
            >
              ...
              <template #cell-status_badge="{ record }">
                <span class="badge" :class="runStatusClass(record.status)">{{
...
.case-status-text {
  --rg-status-pass-bg: var(--color-lime-45-a10) /* -> --color-lime-45-a10 */;
  --rg-status-fail-bg: var(--color-red-69-a10);
}
.top-bar {
  display: flex;
  align-items: center;
...
.case-header {
  ...
  cursor: pointer;
  border-left: 4px solid var(--c-workflow);
}
.case-card.bug-card .case-header {
...
.case-body {
  border-top: 1.5px solid var(--app-border-light);
...
.step-card.step-done .step-strip {
