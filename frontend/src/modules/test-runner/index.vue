...
async function saveTaskToServer(task) {
  try {
    const body = {
      id: task.id, name: task.name, mode: task.mode, deviceSerial: task.deviceSerial,
      caseIds: task.caseIds, loopCount: task.loopCount, intervalSeconds: task.intervalSeconds, running: task.running,
      runId: task.runId || '', caseItems: task.caseItems, stepStates: task.stepStates,
      overallPass: task.overallPass, overallFail: task.overallFail,
      logs: (task.logs || []).slice(-200),
      createdAt: task.createdAt, creator: task.creator,
      currentCaseTitle: task.currentCaseTitle, currentIteration: task.currentIteration,
      failedSteps: (task.failedSteps || []).slice(-200),
      conclusion: task.conclusion || '', bugTicket: task.bugTicket || '',
...
function deriveTaskStatus(task) {
  ...
  if (task.caseItems?.length && task.caseItems[0]?.status === 'pending' && !task.runId) return 'queued'
  // Any terminal outcome means the task is done (completed, stopped, interrupted, error)
  if (task.outcome === 'completed') return 'done'
  if (task.outcome && ['stopped', 'interrupted', 'error'].includes(task.outcome)) return 'done'
  return 'idle'
}
...
const completedTasks = computed(() => tasks.value.filter(t => !t.running && t.outcome === 'completed'))
const incompleteTasks = computed(() => tasks.value.filter(t => taskBucket(t) === 'incomplete'))
const notExecutedTasks = computed(() => tasks.value.filter(t => taskBucket(t) === 'notExecuted'))
...
function taskStatusInfo(task) {
  ...
  if (isTaskQueued(task)) return { label: '等待中', color: '#f7cd67', icon: '⏳' }
  if (task.outcome === 'stopped') return { label: '未完成', color: '#e85f5f', icon: '⏹' }
  if (task.outcome === 'interrupted') return { label: '运行中断', color: '#f7a8c4', icon: '⚠️' }
  if (task.outcome === 'error') return { label: '异常终止', color: '#e85f5f', icon: '💥' }
  if (task.outcome === 'completed') return { label: '已完成', color: '#6fba2c', icon: '✅' }
...
// ── New task form ──
const newForm = ref({ name: '', deviceSerial: '', caseIds: [], loopCount: 3, intervalSeconds: 5, mode: 'now', startAt: '', endAt: '' })
function resetNewForm() { newForm.value = { name: '', deviceSerial: devices.value[0]?.serial || '', caseIds: [], loopCount: 3, intervalSeconds: 5, mode: 'now', startAt: '', endAt: '' } }
...
async function createAndStart() {
  if (!newForm.value.name.trim()) { ElMessage.warning('请输入任务名称'); return }
  ...
  if (newForm.value.intervalSeconds < 5) { ElMessage.warning('轮间间隔最小为 5 秒，请重新设置'); return }
  const tid = generateTaskId()
  const task = {
    id: tid, name: newForm.value.name.trim(), mode: newForm.value.mode,
    ...
    loopCount: newForm.value.loopCount, intervalSeconds: newForm.value.intervalSeconds, running: false, runId: '', status: 'idle',
  ...
  task.caseItems = cases.value
    ...
    .map((c, i) => {
      ...
      return {
        ...
        pass: 0, fail: 0, total: task.loopCount, rate: 0, steps,
      }
    })
  task.overallPass = 0
  task.overallFail = 0
...
async function doStartTask(task) {
  ...
  const body = { case_ids: task.caseIds, loop_count: task.loopCount, interval_seconds: task.intervalSeconds, device_serial: task.deviceSerial, client_task_id: task.id }
  ...
  try {
    ...
    if (data.ok && data.runs?.[0]?.run_id) {
      ...
      if (idx !== -1) {
        tasks.value[idx] = { ...tasks.value[idx], running: true, runId: data.runs[0].run_id, status: 'running' }
...
async function stopTask(task) {
  if (!task.runId) {
    ...
    try { await ElMessageBox.confirm('确定取消该排队任务？任务将回到未执行列表', '取消排队', { confirmButtonText: '取消排队', cancelButtonText: '返回', type: 'warning' }) } catch (_) { return }
    try {
      await client.post('/runner/queue/cancel', { client_task_id: task.id, device_serial: task.deviceSerial })
    } catch (_) { /* backend may return 404 if already started */ }
...
async function removeTask(task) {
  ...
  try { await client.delete(`/runner/tasks/${task.id}`) } catch (_) {}
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case 'step_started':
          ...
          task.stepStates = [...(task.stepStates || []).filter(s => s.index !== msg.step_index),
        ...
        case 'run_finished':
          ...
          pollQueuedTasks(); scheduleSave()
          break
        case 'device_error':
          task.running = false; task.status = 'done'; task.outcome = 'error'
  ...
  try {
    ...
    for (const active of data.active) {
      ...
      if (task && !task.running) {
        ...
        if (idx !== -1) {
          const updated = { ...tasks.value[idx], running: true, runId: active.run_id, status: 'running' }
              ...
              <div v-if="tasksForTab(tab.key).length" class="card-grid">
                ...
                >
                  ...
                  <div class="tc-row1">
                    <span class="tc-name">
                      {{ task.name || task.id }}
                      <span v-if="task.round" class="tc-round-badge">第{{ task.round }}轮</span>
                    </span>
                  ...
                  <div class="tc-row3">
                    <span class="tc-progress-text">
                      已执行 {{ taskCompletedCount(task) }}/{{ taskTotalCount(task) }} 次
                      <template v-if="task.running && task.currentIteration">（第 {{ task.currentIteration }} 轮循环）</template>
                      <template v-if="task.outcome === 'completed'"> · ✅ 全部通过</template>
                      <template v-if="task.outcome === 'stopped'"> · ⏹ 已中止</template>
                    </span>
                  </div>
                  ...
                  <div class="tc-actions" @click.stop>
                    ...
                    <el-button v-else-if="isTaskQueued(task)" size="small" type="warning" @click="stopTask(task)">⏸ 取消排队</el-button>
      ...
      <div class="new-task-form">
        <el-form label-width="88px" label-position="right">
          ...
          <el-form-item label="轮间间隔">
            <el-input-number v-model="newForm.intervalSeconds" :min="5" :max="300" style="width:160px" />
            <span style="margin-left:8px;font-size:12px;color:#9f927d;">秒（最小 5s）</span>
          </el-form-item>
...
.runner-section .doc-section__header { flex-shrink: 0; }
