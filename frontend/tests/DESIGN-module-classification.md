...
- 模块与优先级：对 `testResults[].name` 正则 `tests[\\/]([^\\/]+)[\\/](p[01])[\\/]`；
  匹配不到的文件计入 `summary.unclassified` 桶（防御性兜底）；**v1 实现期裁剪：未分类数据只统计不渲染分区**（见 §9 v1.1）；
...
3. ~~全量通过时顶部显示成功横幅；有失败时失败分区自动置顶展开~~ — **v1 实现期裁剪**：失败以行内红色底色 + 「失败详情」列呈现（fixture 已覆盖），横幅/置顶展开未实现（见 §9 v1.1）。
...
| v1.1 | 2026-08-13 | 实现期裁剪记录（用户裁决：更新文档而非补实现）：§6.3 未分类只统计不渲染；§6.4 成功横幅/失败置顶展开未实现，失败以行内呈现 |
