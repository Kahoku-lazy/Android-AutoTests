...
class StepType(Enum):
    ...
    WAIT_EITHER = "wait_either"           # 等待两个元素之一出现(index=0普通, >0索引)
    ...
    RESTART_APP = "restart_app"           # 重启APP(kill+等待+start, 间隔可配)
...
UI_LABELS = {
    ...
    StepType.WAIT_DISAPPEAR: "等待元素出现后消失",
    StepType.WAIT_EITHER: "等待二选一元素",
    StepType.WAIT_TOAST: "等待Toast",
    StepType.VERIFY_TEXT: "验证文本",
    StepType.POLL_TEXT: "轮询文本",
    StepType.SLEEP: "固定等待",
    StepType.KILL_APP: "杀掉APP",
    StepType.START_APP: "启动APP",
    StepType.RESTART_APP: "重启APP",
    StepType.RETRY_CLICK: "点击重试",
    StepType.LOG: "打印日志",
}

# Reverse mapping: step type value string → StepType enum
STEP_TYPE_MAP = {st.value: st for st in StepType}
...
class TestStep:
    ...
    Fields:
        ...
        xpath2:     Secondary XPath (for wait_either: second element)
        ...
        index:      Multi-purpose index —
                    ...
                    - wait_either: 0=normal element, >0=indexed element
    ...
    xpath2: str = ""             # Secondary XPath (wait_either)
