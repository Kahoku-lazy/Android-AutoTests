...
- **WHEN** 已登录用户请求 `device_action` 的 schema
...
设备管理分类下真正操作手机的工具（`list_apps`、`device_action`、`click_ratio`、`drag_ratio`、`xpath_action`）的 `serial` 参数 SHALL 提供候选设备清单。候选 MUST 同时满足三项：对请求者可见（与设备管理列表同一可见性规则）、状态为在线、当前未被占用。MUST NOT 出现在线但被占用（含被执行引擎占用）的设备，MUST NOT 出现请求者不可见的设备。候选项 MUST 含可提交的取值与可读标签（设备名称或型号与序列号）。设备池占用记账工具 `acquire_device` 与 `release_device` MUST NOT 提供候选，其 `serial` 保持自由输入。

#### Scenario: 候选只含在线且未被占用的可见设备

- **WHEN** 已登录用户打开 `device_action` 调试页
