...
TEST_CASES = {
    "login": {
        ...
        "color": "#6fba2c",
        "steps": [
            {"file": "01_login_page_loaded.png", "title": "Step 1: 打开登录页面",
             "selector": "—", "action": "navigate",
             "desc": "浏览器导航到 <code>/login</code>，等待页面完全加载（networkidle）。",
             "expected": "页面显示登录卡片，包含用户名、密码输入框和登录按钮"},
            {"file": "02_username_filled.png", "title": "Step 2: 输入用户名",
             "selector": 'input[placeholder="账号"]', "action": "fill",
             "desc": "定位用户名输入框，填入 <code>admin</code>。红点标注输入框中心位置。",
             "expected": "输入框显示 'admin'"},
            {"file": "03_password_filled.png", "title": "Step 3: 输入密码",
             "selector": '.login-form input[type="password"]', "action": "fill",
             "desc": "定位密码输入框，填入密码。红点标注输入框中心。",
             "expected": "密码框显示圆点占位符"},
            {"file": "04_remember_toggled.png", "title": "Step 4: 切换记住账号",
             "selector": ".form-remember .el-switch", "action": "click",
             "desc": "点击开关组件，切换「记住账号」状态。红点标注开关中心。",
             "expected": "开关变为激活状态"},
            {"file": "05_after_login_click.png", "title": "Step 5: 点击登录按钮",
             "selector": 'text="开始使用"', "action": "click",
    ...
    "register": {
        ...
        "steps": [
            {"file": "01_login_page_loaded.png", "title": "Step 1: 打开登录页面",
             ...
             "expected": "页面显示登录卡片"},
            {"file": "02_switched_to_register.png", "title": "Step 2: 切换到注册模式",
             "selector": 'text="去注册"', "action": "click",
             "desc": "点击「去注册」链接切换到注册模式。红点标注链接位置。",
             "expected": "卡片切换为注册表单，显示 3 个输入框"},
            {"file": "03_username_filled.png", "title": "Step 3: 输入注册用户名",
             "selector": 'input[placeholder*="设置账号"]', "action": "fill",
             "desc": "定位用户名输入框，填入随机生成的用户名。",
             "expected": "输入框显示生成的用户名"},
            {"file": "04_password_filled.png", "title": "Step 4: 输入注册密码",
             "selector": '.login-form input[placeholder*="设置密码"]', "action": "fill",
             "desc": "定位密码输入框，填入密码（>=6位）。",
             "expected": "密码框显示圆点占位符"},
            {"file": "05_confirm_password_filled.png", "title": "Step 5: 确认密码",
             "selector": 'input[placeholder="确认密码"]', "action": "fill",
             "desc": "定位确认密码输入框，填入相同密码。两次输入必须一致。",
             "expected": "确认密码框已填入，注册按钮变为可点击"},
            {"file": "06_after_register_click.png", "title": "Step 6: 点击注册按钮",
             "selector": 'text="完成注册"', "action": "click",
...
def main():
    ...
    print(f"     {len(cases)} test cases, {sum(c['captured'] for c in cases)} screenshots embedded, {size_mb:.1f} MB")
    for c in cases:
