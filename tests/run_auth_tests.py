...
from apps.test_runner.models import TestResult as TR, TestRunRecord
...
def build_testcase(row, task_type):
    ...
    return TestCaseDef(
        id=row.id, title=row.title, steps_data=steps_data,
        task_type=task_type, extra_data=extra,
    )
...
def _get_results(run_id):
    return list(TR.objects.filter(run__run_id=run_id).values(
...
async def execute_and_collect(case_ids, task_type, label):
    ...
    try:
        await _execute_unified_remote(
            run_id, test_cases, 1, 5,
            client_task_id=run_id, task_type=task_type, device_label=device_label,
        )
    except Exception as e:
