...
from apps.element_locator.models import WebGroup, WebElement
...
def add_els(group, elements):
    ...
    for name, lt, lv, url, desc in elements:
        objs.append(WebElement(group=group, name=name, locator_type=lt, locator_value=lv, page_url=url, description=desc))
...
add_els(g, [
    ...
    ('在线设备卡片', 'css_selector', '.stats-card--green', '/dashboard', '统计卡片: 在线设备, 点击进入设备管理'),
    ('测试用例卡片', 'css_selector', '.stats-card--teal', '/dashboard', '统计卡片: 测试用例, 点击进入用例管理'),
    ('活跃智能体卡片', 'css_selector', '.stats-card--blue', '/dashboard', '统计卡片: 活跃智能体, 点击进入AI助手'),
    ('运行中任务卡片', 'css_selector', '.stats-card--pink', '/dashboard', '统计卡片: 运行中任务, 点击进入执行引擎'),
    ('卡片进入按钮', 'css_selector', '.stats-card__enter', '/dashboard', '各统计卡片上的进入按钮'),
    ('在线设备数值', 'css_selector', '.stats-card--green .stats-card__stat strong', '/dashboard', '在线设备数'),
    ('测试用例数值', 'css_selector', '.stats-card--teal .stats-card__stat strong', '/dashboard', '用例总数'),
    ('运行中任务数值', 'css_selector', '.stats-card--pink .stats-card__stat strong', '/dashboard', '活跃任务数'),
])

# 1b. Trends
g = make_group('趋势图表', parent=db_folder)
add_els(g, [
    ('趋势柱状图', 'css_selector', '.trends-chart-card', '/dashboard', 'ECharts 柱状图: 成功/失败/新建 三系列'),
    ('任务结果面板', 'css_selector', '.trends-tasks-card', '/dashboard', '近期任务执行结果列表'),
...
add_els(g, [
    ('仪表盘入口卡片', 'css_selector', '.module-card--yellow', '/dashboard', '模块导航: 仪表盘快捷入口'),
    ('设备管理入口卡片', 'css_selector', '.module-card--green', '/dashboard', '模块导航: 设备管理入口'),
    ('元素定位入口卡片', 'css_selector', '.module-card--purple', '/dashboard', '模块导航: 元素定位入口'),
    ('用例管理入口卡片', 'css_selector', '.module-card--teal', '/dashboard', '模块导航: 用例管理入口'),
    ('执行引擎入口卡片', 'css_selector', '.module-card--pink', '/dashboard', '模块导航: 执行引擎入口'),
    ('测试报告入口卡片', 'css_selector', '.module-card--brown', '/dashboard', '模块导航: 测试报告入口'),
    ('AI助手入口卡片', 'css_selector', '.module-card--orange', '/dashboard', '模块导航: AI助手入口'),
    ('工作流入口卡片', 'css_selector', '.module-card--blue', '/dashboard', '模块导航: 工作流工作台入口'),
])
...
add_els(g, [
    ...
    ('时间线项-成功', 'css_selector', '.timeline-item--success', '/dashboard', '活动时间线: 成功类型'),
    ('时间线项-警告', 'css_selector', '.timeline-item--warning', '/dashboard', '活动时间线: 警告类型'),
    ('时间线项-错误', 'css_selector', '.timeline-item--error', '/dashboard', '活动时间线: 错误类型'),
    ('系统状态文本', 'css_selector', '.dashboard__footer', '/dashboard', '页脚: 最近更新时间和系统状态'),
])
...
add_els(g, [
    ...
    ('设备搜索框', 'css_selector', '.toolbar input[placeholder*="搜索"]', '/devices', '搜索设备序列号或型号'),
...
add_els(g, [
    ('IP地址输入框', 'css_selector', 'input[placeholder*="IP"]', '/devices', '局域网连接: IP地址输入'),
    ('端口输入框', 'css_selector', 'input[value="5555"]', '/devices', '局域网连接: 端口号, 默认5555'),
...
add_els(g, [
    ('用例标题输入', 'css_selector', 'input[placeholder*="用例标题"]', '/cases/new', '新建/编辑用例: 标题'),
    ('用例ID输入', 'css_selector', 'input[placeholder*="留空则自动生成"]', '/cases/new', '新建用例: 自动或手动ID'),
    ('优先级选择', 'css_selector', 'input[placeholder*="P0"]', '/cases/new', '用例优先级: P0/P1/P2'),
    ...
    ('元素库搜索', 'css_selector', '.xpath-picker input', '/cases/new', '搜索已保存的Android元素'),
    ('弹窗监视器面板', 'css_selector', '.watcher-panel', '/cases/new', '全局弹窗监视器配置'),
])

# 3b. Storage / 3c. API / 3d. Web
for cat_key, cat_name, cols in [
    ('storage', '存储业务', ['测试用例编号', '用例标题', '优先级', '前置条件', '用例步骤', '预期结果']),
    ('api', 'API接口', ['测试用例编号', '用例标题', '优先级', '前置条件', '请求头', '请求体', '预期响应文本']),
    ('web', 'Web自动化', ['测试用例编号', '用例标题', '优先级', '前置条件', '目标URL', '操作步骤', '预期结果']),
]:
    ...
    add_els(g, [
        (f'{cat_name}添加行按钮', 'text', '添加行', f'/cases/{cat_key}/new', f'{cat_name}编辑: 添加数据行'),
        (f'{cat_name}添加列按钮', 'text', '添加列', f'/cases/{cat_key}/new', f'{cat_name}编辑: 添加自定义列'),
        (f'{cat_name}保存按钮', 'text', '保存', f'/cases/{cat_key}/new', f'{cat_name}编辑: 保存'),
    ] + [(f'{cat_name}列-{c}', 'css_selector', f'.cell-input', f'/cases/{cat_key}/new', f'{cat_name}编辑: {c}列') for c in cols])
...
add_els(g, [
    ...
    ('任务名称输入', 'css_selector', 'input[placeholder*="任务名称"]', '/runner', '新建任务: 任务名称'),
    ('设备选择', 'css_selector', 'input[placeholder*="选择在线设备"]', '/runner', '新建任务: 选择设备'),
    ('用例选择', 'css_selector', 'input[placeholder*="选择用例"]', '/runner', '新建任务: 选择用例'),
...
add_els(g, [
    ...
    ('用例执行卡片', 'css_selector', '.case-execution-card', '/runner/task/:id', '单个用例的执行结果'),
...
add_els(g, [
    ...
    ('任务名称搜索', 'css_selector', 'input[placeholder*="任务名称"]', '/reports', '按任务名称搜索'),
...
add_els(g, [
    ...
    ('智能体便签卡片', 'css_selector', '.sticky-note', '/ai-assistant', '智能体便签: 名称/模型/状态'),
...
add_els(g, [
    ('名称输入', 'css_selector', 'input[placeholder*="名称"]', '/ai-assistant/agent/:id', '智能体配置: 名称'),
    ('模型提供商选择', 'css_selector', 'input[placeholder*="模型提供商"]', '/ai-assistant/agent/:id', '智能体配置: 模型提供商'),
    ('APIKey输入', 'css_selector', 'input[type="password"]', '/ai-assistant/agent/:id', '智能体配置: API Key'),
    ('系统提示词输入', 'css_selector', 'textarea[placeholder*="提示词"]', '/ai-assistant/agent/:id', '智能体配置: 系统提示词'),
    ('保存配置按钮', 'text', '保存', '/ai-assistant/agent/:id', '保存智能体配置'),
    ('阶段1工具配置', 'text', '阶段 1', '/ai-assistant/agent/:id', 'SOP阶段1: 需求分析与用例设计'),
...
add_els(g, [
    ('对话消息输入框', 'css_selector', '.chat-input textarea', '/ai-assistant/chat/:id', 'AI对话: 消息输入'),
    ('发送按钮', 'css_selector', '.chat-input button', '/ai-assistant/chat/:id', 'AI对话: 发送消息'),
    ('对话列表', 'css_selector', '.conversation-list', '/ai-assistant/chat/:id', 'AI对话: 历史对话列表'),
    ('消息气泡', 'css_selector', '.message-bubble', '/ai-assistant/chat/:id', 'AI对话: 消息气泡'),
    ('思考块', 'css_selector', '.thinking-block', '/ai-assistant/chat/:id', 'AI对话: 思考过程展示'),
    ('工具调用卡片', 'css_selector', '.tool-call-card', '/ai-assistant/chat/:id', 'AI对话: 工具调用结果'),
])
...
add_els(g, [
    ...
    ('目录名称输入', 'css_selector', 'input[maxlength="80"]', '/workflow', '新建目录: 名称输入'),
])
...
add_els(g, [
    ...
    ('元素搜索框', 'css_selector', 'input[placeholder*="搜索"]', '/elements', '搜索text/resource-id/class'),
...
add_els(g, [
    ('项目分组树', 'css_selector', '.web-element-manager .tree-panel', '/elements', 'Web元素分组织树'),
